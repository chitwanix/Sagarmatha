...
// This module provides functionality for driving Sagarmatha user interface
 ...
 *
 * Used within an automation script to pause the the execution of the
 * current script until Sagarmatha is completely idle. Use as
 * 'yield Scripting.waitLeisure();'
 */
...
const PerfHelperIface = {
    name: 'org.Sagarmatha.PerfHelper',
...
PerfHelper.prototype = {
     _init: function() {
         DBus.session.proxifyObject(this, 'org.Sagarmatha.PerfHelper', '/org/Sagarmatha/PerfHelper');
     }
};
 ...
 *
 * Creates a window using sagarmatha-perf-helper for testing purposes.
...
function defineScriptEvent(name, description) {
    Sagarmatha.PerfLog.get_default().define_event("script." + name,
                                             description,
                                             "");
}
...
function scriptEvent(name) {
    Sagarmatha.PerfLog.get_default().event("script." + name);
}
...
function collectStatistics() {
    Sagarmatha.PerfLog.get_default().collect_statistics();
}
...
function _collect(scriptModule, outputFile) {
    ...
    Sagarmatha.PerfLog.get_default().replay(
    ...
    if (outputFile) {
        ...
        Sagarmatha.write_string_to_stream (out, "{\n");

        Sagarmatha.write_string_to_stream(out, '"events":\n');
        Sagarmatha.PerfLog.get_default().dump_events(out);
        ...
        for (let i = 0; i < monitors.length; i++) {
            ...
            if (i != 0)
                Sagarmatha.write_string_to_stream(out, ', ');
            Sagarmatha.write_string_to_stream(out, '"%s%dx%d+%d+%d"'.format(i == primary ? "*" : "",
        ...
        Sagarmatha.write_string_to_stream (out, ',\n"log":\n');
        Sagarmatha.PerfLog.get_default().dump_log(out);
 ...
 * event log, Sagarmatha will exit.
 **/
function runPerfScript(scriptModule, outputFile) {
    Sagarmatha.PerfLog.get_default().set_enabled(true);
