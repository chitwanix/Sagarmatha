...
/**
 * const Type:
 * @EXTENSION: Sagarmatha extensions
 * @APPLET: Sagarmatha panel applets
 * 
 ...
 * @folder: The folder name within the system and user sagarmatha folders
...
const Type = {
    EXTENSION: {
        ...
        requiredProperties: ['uuid', 'name', 'description', 'sagarmatha-version'],
...
Extension.prototype = {
    ...
    loadMetaData: function(metadataFile) {
        ...
        try {
            let metadataContents = Sagarmatha.get_file_contents_utf8_sync(metadataFile.get_path());
    ...
    validateMetaData: function() {
        ...
        // If sagarmatha or js version are set, check them
        if('sagarmatha-version' in this.meta && !versionCheck(this.meta['sagarmatha-version'], Config.PACKAGE_VERSION)) {
            throw this.logError('Extension is not compatible with current Sagarmatha version', null, State.OUT_OF_DATE);
        }
...
function findExtensionDirectory(uuid, type) {
    ...
    if (directory == null) {
        ...
        for (let i = 0; i < systemDataDirs.length; i++) {
            let dirPath = systemDataDirs[i] + '/sagarmatha/' + type.folder;
