...
ExpoWindowClone.prototype = {
    ...
    onButtonRelease : function (actor, event) {
        if ((Sagarmatha.get_event_state(event) & Clutter.ModifierType.BUTTON1_MASK) ||
            (Sagarmatha.get_event_state(event) & Clutter.ModifierType.BUTTON3_MASK))
        {
            this.emit('selected', event.get_time());
        }
        if ((Sagarmatha.get_event_state(event) & Clutter.ModifierType.BUTTON2_MASK))
        {
    ...
    _init : function(metaWorkspace, box) {
        ...
        this.actor.connect('button-release-event', Lang.bind(this,
            function(actor, event) {
                ...
                let evstate = Sagarmatha.get_event_state(event);
...
ExpoThumbnailsBox.prototype = {
    _init: function() {
        this.actor = new Sagarmatha.GenericContainer({ style_class: 'workspace-thumbnails',
                                                   reactive: true,
    ...
    handleKeyPressEvent: function(actor, event) {
        let modifiers = Sagarmatha.get_event_state(event);
    ...
    allocate: function(actor, box, flags) {
        ...
        let buttonOverlap = this.button.get_theme_node().get_length('-sagarmatha-close-overlap');
