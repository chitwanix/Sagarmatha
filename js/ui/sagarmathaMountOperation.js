...
function SagarmathaMountOperation(source, params) {
    this._init(source, params);
}

SagarmathaMountOperation.prototype = {
    _init: function(source, params) {
        ...
        this.mountOp = new Sagarmatha.MountOperation();
        ...
        this._icon = new St.Icon({ gicon: source.get_icon(),
                                   style_class: 'sagarmatha-mount-operation-icon' });
    },

    _onAskQuestion: function(op, message, choices) {
        this._dialog = new SagarmathaMountQuestionDialog(this._icon);
    ...
    _onAskPassword: function(op, message) {
        ...
        this._source = new SagarmathaMountPasswordSource(message, this._icon, this._reaskPassword);
    ...
    _onShowProcesses2: function(op) {
        ...
        if (!this._processesDialog) {
            this._processesDialog = new SagarmathaProcessesDialog(this._icon);
...
function SagarmathaMountQuestionDialog(icon) {
    this._init(icon);
}

SagarmathaMountQuestionDialog.prototype = {
...
Signals.addSignalMethods(SagarmathaMountQuestionDialog.prototype);

function SagarmathaMountPasswordSource(message, icon, reaskPassword) {
    this._init(message, icon, reaskPassword);
}

SagarmathaMountPasswordSource.prototype = {
    ...
    _init: function(message, icon, reaskPassword) {
        ...
        this._notification = new SagarmathaMountPasswordNotification(this, strings, icon, reaskPassword);
...
Signals.addSignalMethods(SagarmathaMountPasswordSource.prototype);

function SagarmathaMountPasswordNotification(source, strings, icon, reaskPassword) {
    this._init(source, strings, icon, reaskPassword);
}

SagarmathaMountPasswordNotification.prototype = {
...
function SagarmathaProcessesDialog(icon) {
    this._init(icon);
}

SagarmathaProcessesDialog.prototype = {
    __proto__: ModalDialog.ModalDialog.prototype,
    ...
    _setAppsForPids: function(pids) {
        ...
        pids.forEach(Lang.bind(this, function(pid) {
            let tracker = Sagarmatha.WindowTracker.get_default();
...
Signals.addSignalMethods(SagarmathaProcessesDialog.prototype);
