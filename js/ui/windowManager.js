...
function getDimShaderSource() {
    if (!dimShader)
        dimShader = Sagarmatha.get_file_contents_utf8_sync(global.datadir + '/shaders/dim-window.glsl');
    return dimShader;
}
...
WindowDimmer.prototype = {
    ...
    set dimFraction(fraction) {
        ...
        if (fraction > 0.01) {
            Sagarmatha.shader_effect_set_double_uniform(this._effect, 'height', this.actor.get_height());
            Sagarmatha.shader_effect_set_double_uniform(this._effect, 'fraction', fraction);
...
WindowManager.prototype = {
    _init : function() {
        this._sagarmathawm =  global.window_manager;
        ...
        this._sagarmathawm.connect('kill-switch-workspace', Lang.bind(this, this._switchWorkspaceDone));
        this._sagarmathawm.connect('kill-window-effects', Lang.bind(this, function (sagarmathawm, actor) {
            this._minimizeWindowDone(sagarmathawm, actor);
            this._maximizeWindowDone(sagarmathawm, actor);
            this._unmaximizeWindowDone(sagarmathawm, actor);
            this._tileWindowDone(sagarmathawm, actor);
            this._mapWindowDone(sagarmathawm, actor);
            this._destroyWindowDone(sagarmathawm, actor);
        }));

        this._sagarmathawm.connect('switch-workspace', Lang.bind(this, this._switchWorkspace));
        this._sagarmathawm.connect('minimize', Lang.bind(this, this._minimizeWindow));
        this._sagarmathawm.connect('maximize', Lang.bind(this, this._maximizeWindow));
        this._sagarmathawm.connect('unmaximize', Lang.bind(this, this._unmaximizeWindow));
        this._sagarmathawm.connect('tile', Lang.bind(this, this._tileWindow));
        this._sagarmathawm.connect('map', Lang.bind(this, this._mapWindow));
        this._sagarmathawm.connect('destroy', Lang.bind(this, this._destroyWindow));
    ...
    _fadeWindow: function(sagarmathawm, actor, opacity, time, transition, callbackComplete, callbackOverwrite) {        
        ...
        Tweener.addTween(actor,
                         { opacity: opacity,                               
                           ...
                           onCompleteParams: [sagarmathawm, actor],
                           onOverwrite: callbackOverwrite,
                           onOverwriteScope: this,
                           onOverwriteParams: [sagarmathawm, actor]
                            });
    },
    
    _scaleWindow: function(sagarmathawm, actor, scale_x, scale_y, time, transition, callbackComplete, callbackOverwrite) {        
        actor.move_anchor_point_from_gravity(Clutter.Gravity.CENTER);        
        Tweener.addTween(actor,
                         { scale_x: scale_x,     
                           scale_y: scale_y,                          
    ...
    _minimizeWindow : function(sagarmathawm, actor) {

        Main.soundManager.play('minimize');

        // reset all cached values in case "traditional" is no longer in effect
        actor.get_meta_window()._sagarmathawm_has_origin = false;
        actor.get_meta_window()._sagarmathawm_minimize_transition = undefined;
        actor.get_meta_window()._sagarmathawm_minimize_time = undefined;

        if (!this._shouldAnimate(actor)) {
            sagarmathawm.completed_minimize(actor);
            return;
        }
        ...
        if (effect == "traditional") {
            if (AppletManager.get_role_provider_exists(AppletManager.Roles.WINDOWLIST)) {
                ...
                if (actorOrigin !== false) {
                    ...
                    actor.get_meta_window()._sagarmathawm_has_origin = true;
                    actor.get_meta_window()._sagarmathawm_minimize_transition = transition;
                    actor.get_meta_window()._sagarmathawm_minimize_time = time;
                    Tweener.addTween(actor,
                                     { scale_x: 0.0,
                                       scale_y: 0.0,
                                       x: xDest,
        ...
        if (effect == "fade") {
            this._minimizing.push(actor);
            this._fadeWindow(sagarmathawm, actor, 0, time, transition, this._minimizeWindowDone, this._minimizeWindowOverwritten);            
        } else if (effect == "scale") {                                
            this._minimizing.push(actor);
            this._scaleWindow(sagarmathawm, actor, 0.0, 0.0, time, transition, this._minimizeWindowDone, this._minimizeWindowOverwritten); 
        } else {
            sagarmathawm.completed_minimize(actor);
        }
    },

    _minimizeWindowDone : function(sagarmathawm, actor) {
    ...
    _minimizeWindowOverwritten : function(sagarmathawm, actor) {
        if (this._removeEffect(this._minimizing, actor)) {
            sagarmathawm.completed_minimize(actor);
        }
    },

    _tileWindow : function (sagarmathawm, actor, targetX, targetY, targetWidth, targetHeight) {
        Main.soundManager.play('tile');
        if (!this._shouldAnimate(actor)) {
            sagarmathawm.completed_tile(actor);
            return;
        }
    ...
    _tileWindowDone : function(sagarmathawm, actor) {
    ...
    _tileWindowOverwrite : function(sagarmathawm, actor) {
        if (this._removeEffect(this._tiling, actor)) {
            sagarmathawm.completed_tile(actor);
        }
    },

    _maximizeWindow : function(sagarmathawm, actor, targetX, targetY, targetWidth, targetHeight) {
        Main.soundManager.play('maximize');
        if (!this._shouldAnimate(actor)) {
            sagarmathawm.completed_maximize(actor);
            return;
        }
    ...
    _maximizeWindowDone : function(sagarmathawm, actor) {
    ...
    _maximizeWindowOverwrite : function(sagarmathawm, actor) {
        if (this._removeEffect(this._maximizing, actor)) {
            sagarmathawm.completed_maximize(actor);
        }
    },

    _unmaximizeWindow : function(sagarmathawm, actor, targetX, targetY, targetWidth, targetHeight) {
        Main.soundManager.play('unmaximize');
        if (!this._shouldAnimate(actor)) {
            sagarmathawm.completed_unmaximize(actor);
            return;
        }
    ...
    _unmaximizeWindowDone : function(sagarmathawm, actor) {
    ...
    _unmaximizeWindowOverwrite : function(sagarmathawm, actor) {
        if (this._removeEffect(this._unmaximizing, actor)) {
            sagarmathawm.completed_unmaximize(actor);
        }
    },
    ...
    _mapWindow : function(sagarmathawm, actor) {           
        ...
        if (actor.meta_window.is_attached_dialog()) {
            ...
            sagarmathawm.completed_map(actor);
            return;
        }
        if (!this._shouldAnimate(actor)) {
            this._completeMap(sagarmathawm, actor);
            return;
        }
        ...
        if (actor.get_meta_window()._sagarmathawm_has_origin === true) {
            ...
            if (AppletManager.get_role_provider_exists(AppletManager.Roles.WINDOWLIST))
            {
                ...
                if (actorOrigin !== false) {
                    ...
                    let myTransition = actor.get_meta_window()._sagarmathawm_minimize_transition||transition;
                    let lastTime = actor.get_meta_window()._sagarmathawm_minimize_time;
        ...
        if (effect == "fade") {            
            ...
            actor.opacity = 0;
            actor.show();
            this._fadeWindow(sagarmathawm, actor, 255, time, transition, this._mapWindowDone, this._mapWindowOverwrite);
        }
        else if (effect == "scale") {
            actor.set_scale(0.0, 0.0);
            actor.show();
            this._mapping.push(actor);
            this._scaleWindow(sagarmathawm, actor, 1, 1, time, transition, this._mapWindowDone, this._mapWindowOverwrite);        
        }
        else {   
            this._completeMap(sagarmathawm, actor);
        }
    ...
    _completeMap : function(sagarmathawm, actor) {
        actor.move_anchor_point_from_gravity(Clutter.Gravity.NORTH_WEST);
        actor.set_scale(1.0, 1.0);
        actor.opacity = 255;
        actor.show();
    ...
    _mapWindowDone : function(sagarmathawm, actor) {
    ...
    _mapWindowOverwrite : function(sagarmathawm, actor) {
        if (this._removeEffect(this._mapping, actor)) {
            this._completeMap(sagarmathawm, actor);
        }
    },

    _destroyWindow : function(sagarmathawm, actor) {  
        ...
        if (window.is_attached_dialog()) {
            ...
            if (!this._shouldAnimate()) {
                sagarmathawm.completed_destroy(actor);
                return;
            }
        ...
        if (effect == "scale") {
            this._destroying.push(actor);
            this._scaleWindow(sagarmathawm, actor, 0.0, 0.0, time, transition, this._destroyWindowDone, this._destroyWindowDone);
        }
        else if (effect == "fade") {
            this._destroying.push(actor);
                //Fixes ghost windows bug
            Tweener.removeTweens(actor);
            this._fadeWindow(sagarmathawm, actor, 0, time, transition, this._destroyWindowDone, this._destroyWindowDone);
        }
        else {
            sagarmathawm.completed_destroy(actor);
        }
    ...
    _destroyWindowDone : function(sagarmathawm, actor) {
    ...
    _switchWorkspace : function(sagarmathawm, from, to, direction) {
        if (!this._shouldAnimate()) {
            sagarmathawm.completed_switch_workspace();                                
            return;
        }
    ...
    _switchWorkspaceDone : function(sagarmathawm) {
    ...
    _showSnapOSD : function() {
        if (!global.settings.get_boolean("hide-snap-osd")) {
            if (this._snap_osd == null) {
                ...
                let settings = new Gio.Settings({ schema: "org.sagarmatha.muffin" });
