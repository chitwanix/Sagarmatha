...
#include "sagarmatha-embedded-window-private.h"
...
struct _SagarmathaGtkEmbedPrivate
{
  SagarmathaEmbeddedWindow *window;
};

G_DEFINE_TYPE (SagarmathaGtkEmbed, sagarmatha_gtk_embed, CLUTTER_X11_TYPE_TEXTURE_PIXMAP);

static void sagarmatha_gtk_embed_set_window (SagarmathaGtkEmbed       *embed,
                                        SagarmathaEmbeddedWindow *window);

static void
sagarmatha_gtk_embed_on_window_destroy (GtkWidget     *object,
...
static void
sagarmatha_gtk_embed_on_window_realize (GtkWidget     *widget,
                                   SagarmathaGtkEmbed *embed)
{
...
static void
sagarmatha_gtk_embed_set_window (SagarmathaGtkEmbed       *embed,
                            SagarmathaEmbeddedWindow *window)
{

  if (embed->priv->window)
    {
      _sagarmatha_embedded_window_set_actor (embed->priv->window, NULL);
      ...
      g_signal_handlers_disconnect_by_func (embed->priv->window,
                                            (gpointer)sagarmatha_gtk_embed_on_window_destroy,
                                            embed);
      g_signal_handlers_disconnect_by_func (embed->priv->window,
                                            (gpointer)sagarmatha_gtk_embed_on_window_realize,
                                            embed);
    }
  ...
  if (embed->priv->window)
    {
      ...
      _sagarmatha_embedded_window_set_actor (embed->priv->window, embed);

      g_signal_connect (embed->priv->window, "destroy",
                        G_CALLBACK (sagarmatha_gtk_embed_on_window_destroy), embed);
      g_signal_connect (embed->priv->window, "realize",
                        G_CALLBACK (sagarmatha_gtk_embed_on_window_realize), embed);

      if (gtk_widget_get_realized (GTK_WIDGET (window)))
        sagarmatha_gtk_embed_on_window_realize (GTK_WIDGET (embed->priv->window), embed);
    }
...
static void
sagarmatha_gtk_embed_set_property (GObject         *object,
                              guint            prop_id,
                              const GValue    *value,
                              GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_WINDOW:
      sagarmatha_gtk_embed_set_window (embed, (SagarmathaEmbeddedWindow *)g_value_get_object (value));
      break;
...
static void
sagarmatha_gtk_embed_get_property (GObject         *object,
                              guint            prop_id,
                              GValue          *value,
                              GParamSpec      *pspec)
{
...
static void
sagarmatha_gtk_embed_get_preferred_width (ClutterActor *actor,
                                     float         for_height,
                                     float        *min_width_p,
...
static void
sagarmatha_gtk_embed_get_preferred_height (ClutterActor *actor,
                                      float         for_width,
                                      float        *min_height_p,
...
static void
sagarmatha_gtk_embed_allocate (ClutterActor          *actor,
                          ...
                          ClutterAllocationFlags flags)
{
  ...
  _sagarmatha_embedded_window_allocate (embed->priv->window,
...
sagarmatha_gtk_embed_realize (ClutterActor *actor)
{
  ...
  CLUTTER_ACTOR_CLASS (sagarmatha_gtk_embed_parent_class)->realize (actor);
}
...
sagarmatha_gtk_embed_unrealize (ClutterActor *actor)
{
  ...
  CLUTTER_ACTOR_CLASS (sagarmatha_gtk_embed_parent_class)->unrealize (actor);
}
...
sagarmatha_gtk_embed_dispose (GObject *object)
{
  ...
  G_OBJECT_CLASS (sagarmatha_gtk_embed_parent_class)->dispose (object);
}

static void
sagarmatha_gtk_embed_class_init (SagarmathaGtkEmbedClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  ClutterActorClass *actor_class = CLUTTER_ACTOR_CLASS (klass);

  g_type_class_add_private (klass, sizeof (SagarmathaGtkEmbedPrivate));

  object_class->get_property = sagarmatha_gtk_embed_get_property;
  object_class->set_property = sagarmatha_gtk_embed_set_property;
  object_class->dispose      = sagarmatha_gtk_embed_dispose;

  actor_class->get_preferred_width = sagarmatha_gtk_embed_get_preferred_width;
  actor_class->get_preferred_height = sagarmatha_gtk_embed_get_preferred_height;
  actor_class->allocate = sagarmatha_gtk_embed_allocate;
  actor_class->realize = sagarmatha_gtk_embed_realize;
  actor_class->unrealize = sagarmatha_gtk_embed_unrealize;
