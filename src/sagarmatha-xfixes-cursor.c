...
#include "sagarmatha-xfixes-cursor.h"

#include <clutter/x11/clutter-x11.h>
#include <X11/extensions/Xfixes.h>

/**
 * SECTION:SagarmathaXFixesCursor
 * short_description: Capture/manipulate system mouse cursor.
 *
 * The #SagarmathaXFixesCursor object uses the XFixes extension to show/hide the
 * the system mouse pointer, to grab its image as it changes, and emit a
 * notification when its image changes.
 */

struct _SagarmathaXFixesCursorClass
{
  GObjectClass parent_class;
};

struct _SagarmathaXFixesCursor {
  GObject parent;
...
static void xfixes_cursor_show        (SagarmathaXFixesCursor *xfixes_cursor);
static void xfixes_cursor_hide        (SagarmathaXFixesCursor *xfixes_cursor);

static void xfixes_cursor_set_stage   (SagarmathaXFixesCursor *xfixes_cursor,
                                       ClutterStage  *stage);

static void xfixes_cursor_reset_image (SagarmathaXFixesCursor *xfixes_cursor);
...
G_DEFINE_TYPE(SagarmathaXFixesCursor, sagarmatha_xfixes_cursor, G_TYPE_OBJECT);
...
static void
sagarmatha_xfixes_cursor_init (SagarmathaXFixesCursor *xfixes_cursor)
{
...
sagarmatha_xfixes_cursor_finalize (GObject  *object)
{
  ...
  G_OBJECT_CLASS (sagarmatha_xfixes_cursor_parent_class)->finalize (object);
}

static void
xfixes_cursor_on_stage_destroy (ClutterActor  *actor,
                                SagarmathaXFixesCursor *xfixes_cursor)
{
  xfixes_cursor_set_stage (xfixes_cursor, NULL);
}
...
xfixes_cursor_event_filter (XEvent        *xev,
                            ...
                            gpointer       data)
{
  SagarmathaXFixesCursor *xfixes_cursor = data;
...
static void
xfixes_cursor_set_stage (SagarmathaXFixesCursor *xfixes_cursor,
                         ClutterStage  *stage)
{
...
static void
xfixes_cursor_show (SagarmathaXFixesCursor *xfixes_cursor)
{
  int minor, major;
  Display *xdisplay;
  Window xwindow;
...
static void
xfixes_cursor_hide (SagarmathaXFixesCursor *xfixes_cursor)
{
  int minor, major;
  Display *xdisplay;
  Window xwindow;
...
static void
xfixes_cursor_reset_image (SagarmathaXFixesCursor *xfixes_cursor)
{
...
static void
sagarmatha_xfixes_cursor_set_property (GObject      *object,
                                 guint         prop_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
...
static void
sagarmatha_xfixes_cursor_get_property (GObject         *object,
                             guint            prop_id,
                             GValue          *value,
                             GParamSpec      *pspec)
{
...
static void
sagarmatha_xfixes_cursor_class_init (SagarmathaXFixesCursorClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->finalize = sagarmatha_xfixes_cursor_finalize;
  ...
  gobject_class->get_property = sagarmatha_xfixes_cursor_get_property;
  gobject_class->set_property = sagarmatha_xfixes_cursor_set_property;
...
/**
 * sagarmatha_xfixes_cursor_get_for_stage:
 * @stage: (transfer none): The #ClutterStage to get the cursor for
 *
 * Return value: (transfer none): A #SagarmathaXFixesCursor instance
 */
SagarmathaXFixesCursor *
sagarmatha_xfixes_cursor_get_for_stage (ClutterStage *stage)
{
  SagarmathaXFixesCursor *instance;
  static GQuark xfixes_cursor_quark;

  if (G_UNLIKELY (xfixes_cursor_quark == 0))
    xfixes_cursor_quark = g_quark_from_static_string ("sagarmatha-xfixes-cursor");
...
/**
 * sagarmatha_xfixes_cursor_hide:
 * @xfixes_cursor: the #SagarmathaXFixesCursor
 *
...
/**
 * sagarmatha_xfixes_cursor_update_texture_image:
 * @xfixes_cursor:  the #SagarmathaXFixesCursor
 * @texture:        ClutterTexture to update with the current sprite image.
 */
void
sagarmatha_xfixes_cursor_update_texture_image (SagarmathaXFixesCursor *xfixes_cursor,
...
/**
 * sagarmatha_xfixes_cursor_get_hot_x:
 * @xfixes_cursor: the #SagarmathaXFixesCursor
 *
...
/**
 * sagarmatha_xfixes_cursor_get_hot_y:
 * @xfixes_cursor: the #SagarmathaXFixesCursor
 *
