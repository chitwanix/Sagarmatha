 ...
 * SagarmathaGtkEmbed; the window is reparented into the stage
 * window for the actor and the resizing logic is bound to the clutter
 * logic.
 *
 * The typical usage we might expect is
 *
 *  - SagarmathaEmbeddedWindow is created and filled with content
 *  - SagarmathaEmbeddedWindow is shown with gtk_widget_show_all()
 *  - SagarmathaGtkEmbed is created for the SagarmathaEmbeddedWindow
...
G_DEFINE_TYPE (SagarmathaEmbeddedWindow, sagarmatha_embedded_window, GTK_TYPE_WINDOW);
...
struct _SagarmathaEmbeddedWindowPrivate {
...
sagarmatha_embedded_window_hide (GtkWidget *widget)
{
  ...
  GTK_WIDGET_CLASS (sagarmatha_embedded_window_parent_class)->hide (widget);
}
...
static gboolean
sagarmatha_embedded_window_configure_event (GtkWidget         *widget,
...
static void
sagarmatha_embedded_window_set_property (GObject         *object,
                                    guint            prop_id,
                                    const GValue    *value,
                                    GParamSpec      *pspec)
{
...
static GObject *
sagarmatha_embedded_window_constructor (GType                  gtype,
                                   guint                  n_properties,
                                   GObjectConstructParam *properties)
{
  ...
  parent_class = G_OBJECT_CLASS (sagarmatha_embedded_window_parent_class);
...
static void
sagarmatha_embedded_window_class_init (SagarmathaEmbeddedWindowClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);
  GtkContainerClass *container_class = GTK_CONTAINER_CLASS (klass);

  g_type_class_add_private (klass, sizeof (SagarmathaEmbeddedWindowPrivate));

  object_class->set_property    = sagarmatha_embedded_window_set_property;
  object_class->constructor     = sagarmatha_embedded_window_constructor;

  widget_class->show            = sagarmatha_embedded_window_show;
  widget_class->hide            = sagarmatha_embedded_window_hide;
  widget_class->realize         = sagarmatha_embedded_window_realize;
  widget_class->configure_event = sagarmatha_embedded_window_configure_event;

  container_class->check_resize    = sagarmatha_embedded_window_check_resize;
...
/*
 * Private routines called by SagarmathaGtkEmbed
 */

void
_sagarmatha_embedded_window_set_actor (SagarmathaEmbeddedWindow  *window,
                                  SagarmathaGtkEmbed        *actor)
...
void
_sagarmatha_embedded_window_allocate (SagarmathaEmbeddedWindow *window,
                                 int                  x,
                                 int                  y,
                                 int                  width,
                                 int                  height)
