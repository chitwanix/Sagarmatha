...
static GFile *
sagarmatha_util_get_gfile_root (GFile *file)
{
  GFile *parent;
  GFile *parent_old;
...
static char *
sagarmatha_util_get_file_display_name_if_mount (GFile *file)
{
  GFile *compare;
...
static char *
sagarmatha_util_get_file_display_for_common_files (GFile *file)
{
  GFile *compare;
...
static char *
sagarmatha_util_get_file_description (GFile *file)
{
  GFileInfo *info;
  char *ret;
...
static char *
sagarmatha_util_get_file_display_name (GFile *file, gboolean use_fallback)
{
  GFileInfo *info;
  char *ret;
...
static GIcon *
sagarmatha_util_get_file_icon_if_mount (GFile *file)
{
  GFile *compare;
  GVolumeMonitor *monitor;
  GList *mounts, *l;
...
static const char *
sagarmatha_util_get_icon_for_uri_known_folders (const char *uri)
{
  const char *icon;
  char *path;
  int len;
...
char *
sagarmatha_util_get_label_for_uri (const char *text_uri)
{
  GFile *file;
  char  *label;
  GFile *root;
  ...
  label = sagarmatha_util_get_file_display_name_if_mount (file);
  if (label)
    {
      g_object_unref (file);
      return label;
  ...
  if (g_str_has_prefix (text_uri, "file:"))
    {
      label = sagarmatha_util_get_file_display_for_common_files (file);
      if (!label)
        label = sagarmatha_util_get_file_description (file);
      if (!label)
        label = sagarmatha_util_get_file_display_name (file, TRUE);
        g_object_unref (file);
  ...
  root = sagarmatha_util_get_gfile_root (file);
  root_display = sagarmatha_util_get_file_description (root);
  if (!root_display)
    root_display = sagarmatha_util_get_file_display_name (root, FALSE);
  if (!root_display)
    /* can happen with URI schemes non supported by gvfs */
  ...
  if (g_file_equal (file, root))
  ...
  else
    {
      ...
      displayname = sagarmatha_util_get_file_display_name (file, TRUE);
...
/**
 * sagarmatha_util_get_icon_for_uri:
 * @text_uri: A URI
 *
 ...
 */
GIcon *
sagarmatha_util_get_icon_for_uri (const char *text_uri)
{
  const char *name;
  GFile *file;
  GFileInfo *info;
  ...
  name = sagarmatha_util_get_icon_for_uri_known_folders (text_uri);
  if (name)
  ...
  retval = sagarmatha_util_get_file_icon_if_mount (file);
  if (retval)
    return retval;
...
/**
 * sagarmatha_util_set_hidden_from_pick:
  ...
  if (hidden)
    {
      ...
      g_object_set_data (G_OBJECT (actor),
                         "sagarmatha-stop-pick", GUINT_TO_POINTER (1));
    }
  else
    {
      if (existing_handler_data == NULL)
      ...
      g_object_set_data (G_OBJECT (actor), "sagarmatha-stop-pick", NULL);
    }
}

/**
 * sagarmatha_util_get_transformed_allocation:
 * @actor: a #ClutterActor
 ...
 */
void
sagarmatha_util_get_transformed_allocation (ClutterActor    *actor,
                                       ClutterActorBox *box)
{
...
char *
sagarmatha_util_normalize_and_casefold (const char *str)
{
 ...
 */
char *
sagarmatha_util_format_date (const char *format,
                        gint64      time_ms)
{
  GDateTime *datetime;
  GTimeVal tv;
...
/**
 * sagarmatha_util_get_week_start:
 *
...
/**
 * sagarmatha_write_soup_message_to_stream:
 ...
 */
void
sagarmatha_write_soup_message_to_stream (GOutputStream *stream,
                                    SoupMessage   *message,
                                    GError       **error)
{
...
/**
 * sagarmatha_write_string_to_stream:
 * @stream: a #GOutputStream
 ...
 */
gboolean
sagarmatha_write_string_to_stream (GOutputStream *stream,
                              const char    *str,
                              GError       **error)
{
...
/**
 * sagarmatha_get_file_contents_utf8_sync:
 ...
 */
char *
sagarmatha_get_file_contents_utf8_sync (const char *path,
                                   GError    **error)
{
  char *contents;
  gsize len;
 ...
 */
gboolean
sagarmatha_parse_search_provider (const char    *data,
                             char         **name,
                             char         **url,
                             GList        **langs,
                             char         **icon_data_uri,
...
/**
 * sagarmatha_shader_effect_set_double_uniform:
 ...
 */
void
sagarmatha_shader_effect_set_double_uniform (ClutterShaderEffect *effect,
                                        const gchar         *name,
                                        gdouble             value)
{
