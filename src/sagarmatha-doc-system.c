...
#include "sagarmatha-doc-system.h"

#include "sagarmatha-global.h"


/**
 * SECTION:sagarmatha-doc-system
...
struct _SagarmathaDocSystemPrivate {
...
G_DEFINE_TYPE(SagarmathaDocSystem, sagarmatha_doc_system, G_TYPE_OBJECT);

/**
 * sagarmatha_doc_system_get_all:
 * @system: A #SagarmathaDocSystem
 *
 ...
 */
GSList *
sagarmatha_doc_system_get_all (SagarmathaDocSystem    *self)
{
  return self->priv->infos_by_timestamp;
}

/**
 * sagarmatha_doc_system_lookup_by_uri:
 * @system: A #SagarmathaDocSystem
 * @uri: Url
 *
 ...
 */
GtkRecentInfo *
sagarmatha_doc_system_lookup_by_uri (SagarmathaDocSystem  *self,
                                const char      *uri)
{
...
typedef struct {
  SagarmathaDocSystem *self;
  GtkRecentInfo *info;
} SagarmathaDocSystemRecentQueryData;
...
on_recent_file_query_result (GObject       *source,
                             ...
                             gpointer       user_data)
{
  SagarmathaDocSystemRecentQueryData *data = user_data;
  SagarmathaDocSystem *self = data->self;
  GError *error = NULL;
  GFileInfo *fileinfo;
  ...
  if (error != NULL && error->domain == G_IO_ERROR && error->code == G_IO_ERROR_NOT_FOUND)
    {
      ...
      if (self->priv->idle_emit_deleted_id == 0)
        self->priv->idle_emit_deleted_id = g_timeout_add (0, sagarmatha_doc_system_idle_emit_deleted, self);
    }
  g_clear_error (&error);
...
/**
 * sagarmatha_doc_system_queue_existence_check:
 * @system: A #SagarmathaDocSystem
 ...
 * any deleted files will be emitted from the #SagarmathaDocSystem::deleted
 ...
 *
 * The intent of this function is to be called after a #SagarmathaDocSystem::changed
 * signal has been emitted, and a display has shown a subset of those files.
 */
void
sagarmatha_doc_system_queue_existence_check (SagarmathaDocSystem   *self,
                                        guint             n_items)
{
  GSList *iter;
  guint i;
  ...
  for (i = 0, iter = self->priv->infos_by_timestamp; i < n_items && iter; i++, iter = iter->next)
    {
      ...
      const char *uri;
      GFile *file;
      SagarmathaDocSystemRecentQueryData *data;

      if (!gtk_recent_info_is_local (info))
        continue;

      data = g_new0 (SagarmathaDocSystemRecentQueryData, 1);
      data->self = self;
...
static void
sagarmatha_doc_system_on_recent_changed (GtkRecentManager  *manager,
                                    SagarmathaDocSystem    *self)
{
 ...
 */
void
sagarmatha_doc_system_open (SagarmathaDocSystem *system,
                       GtkRecentInfo  *info,
                       int             workspace)
{
  GFile *file;
  ...
  context = sagarmatha_global_create_app_launch_context (sagarmatha_global_get ());
  if (workspace != -1)
...
static void
sagarmatha_doc_system_class_init(SagarmathaDocSystemClass *klass)
{
  ...
  g_type_class_add_private (gobject_class, sizeof (SagarmathaDocSystemPrivate));
}

static void
sagarmatha_doc_system_init (SagarmathaDocSystem *self)
{
  ...
  g_signal_connect (self->priv->manager, "changed", G_CALLBACK(sagarmatha_doc_system_on_recent_changed), self);
  sagarmatha_doc_system_on_recent_changed (self->priv->manager, self);
}

/**
 * sagarmatha_doc_system_get_default:
 *
 * Return Value: (transfer none): The global #SagarmathaDocSystem singleton
 */
SagarmathaDocSystem *
sagarmatha_doc_system_get_default ()
{
