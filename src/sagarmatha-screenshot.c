...
#include "sagarmatha-screen-grabber.h"
#include "sagarmatha-screenshot.h"

struct _SagarmathaScreenshotClass
{
  GObjectClass parent_class;
};

struct _SagarmathaScreenshot
{
...
typedef struct _screenshot_data {
  SagarmathaScreenshot  *screenshot;
...
G_DEFINE_TYPE(SagarmathaScreenshot, sagarmatha_screenshot, G_TYPE_OBJECT);

static void
sagarmatha_screenshot_class_init (SagarmathaScreenshotClass *screenshot_class)
{
  (void) screenshot_class;
}

static void
sagarmatha_screenshot_init (SagarmathaScreenshot *screenshot)
{
  screenshot->global = sagarmatha_global_get ();
}
...
do_grab_screenshot (_screenshot_data *screenshot_data,
                    ...
                    int               height)
{
  SagarmathaScreenGrabber *grabber;
  static const cairo_user_data_key_t key;
  guchar *data;

  grabber = sagarmatha_screen_grabber_new ();
  data = sagarmatha_screen_grabber_grab (grabber, x, y, width, height);
...
{
  MetaScreen *screen = sagarmatha_global_get_screen (screenshot_data->screenshot->global);
  int width, height;
...
/**
 * sagarmatha_screenshot_screenshot:
 * @screenshot: the #SagarmathaScreenshot
 ...
 */
void
sagarmatha_screenshot_screenshot (SagarmathaScreenshot *screenshot,
                             gboolean include_cursor,
                             const char *filename,
                             SagarmathaScreenshotCallback callback)
{
  ...
  stage = CLUTTER_ACTOR (sagarmatha_global_get_stage (screenshot->global));
...
/**
 * sagarmatha_screenshot_screenshot_area:
 * @screenshot: the #SagarmathaScreenshot
 ...
 */
void
sagarmatha_screenshot_screenshot_area (SagarmathaScreenshot *screenshot,
                                  gboolean include_cursor,
                                  int x,
                                  int y,
                                  int width,
...
/**
 * sagarmatha_screenshot_screenshot_window:
 * @screenshot: the #SagarmathaScreenshot
 ...
 */
void
sagarmatha_screenshot_screenshot_window (SagarmathaScreenshot *screenshot,
                                    ...
                                    SagarmathaScreenshotCallback callback)
{
  ...
  MetaScreen *screen = sagarmatha_global_get_screen (screenshot->global);
  ...
  result = g_simple_async_result_new (NULL, on_screenshot_written, (gpointer)screenshot_data, sagarmatha_screenshot_screenshot_window);
  g_simple_async_result_run_in_thread (result, write_screenshot_thread, G_PRIORITY_DEFAULT, NULL);
  g_object_unref (result);
}
