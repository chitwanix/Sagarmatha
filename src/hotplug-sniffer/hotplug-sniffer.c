...
#define BUS_NAME "org.Sagarmatha.HotplugSniffer"
#define AUTOQUIT_TIMEOUT 5

static const gchar introspection_xml[] =
  "<node>"
  "  <interface name='org.Sagarmatha.HotplugSniffer'>"
...
handle_sniff_uri (InvocationData *data)
{
  ...
  sniffer = sagarmatha_mime_sniffer_new (file);
  sagarmatha_mime_sniffer_sniff_async (sniffer,
...
print_debug (const gchar *format, ...)
{
  ...
  g_print ("sagarmatha-hotplug-sniffer[%d]: %s.%03d: %s\n", pid, timebuf, (gint) (now.tv_usec / 1000), s);
  g_free (s);
 out:
  ;
}
