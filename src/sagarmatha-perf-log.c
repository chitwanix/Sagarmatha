...
typedef struct _SagarmathaPerfEvent SagarmathaPerfEvent;
typedef struct _SagarmathaPerfStatistic SagarmathaPerfStatistic;
typedef struct _SagarmathaPerfStatisticsClosure SagarmathaPerfStatisticsClosure;
typedef union  _SagarmathaPerfStatisticValue SagarmathaPerfStatisticValue;
typedef struct _SagarmathaPerfBlock SagarmathaPerfBlock;

/**
 * SECTION:sagarmatha-perf-log
 * @short_description: Event recorder for performance measurement
 *
 * SagarmathaPerfLog provides a way for different parts of the code to
...
struct _SagarmathaPerfLogClass
{
  GObjectClass parent_class;
};

struct _SagarmathaPerfEvent
{
  guint16 id;
  char *name;
  char *description;
...
union _SagarmathaPerfStatisticValue
{
  int i;
  gint64 x;
};

struct _SagarmathaPerfStatistic
{
  SagarmathaPerfEvent *event;

  SagarmathaPerfStatisticValue current_value;
  SagarmathaPerfStatisticValue last_value;

  guint initialized : 1;
  guint recorded : 1;
};

struct _SagarmathaPerfStatisticsClosure
{
...
G_DEFINE_TYPE(SagarmathaPerfLog, sagarmatha_perf_log, G_TYPE_OBJECT);
...
static void
sagarmatha_perf_log_init (SagarmathaPerfLog *perf_log)
{
   ...
   * logging is enabled some time after starting Sagarmatha */
  sagarmatha_perf_log_define_event (perf_log, "perf.setTime", "", "x");
   ...
   */
  sagarmatha_perf_log_define_event (perf_log, "perf.statisticsCollected",
...
static void
sagarmatha_perf_log_class_init (SagarmathaPerfLogClass *class)
{
}

/**
 * sagarmatha_perf_log_get_default:
 *
 * Gets the global singleton performance log. This is initially disabled
 * and must be explicitly enabled with sagarmatha_perf_log_set_enabled().
 *
 * Return value: (transfer none): the global singleton performance log
 */
SagarmathaPerfLog *
sagarmatha_perf_log_get_default (void)
{
...
{
  SagarmathaPerfLog *perf_log = data;

  sagarmatha_perf_log_collect_statistics (perf_log);

  return TRUE;
}

/**
 * sagarmatha_perf_log_set_enabled:
 * @perf_log: a #SagarmathaPerfLog
 * @enabled: whether to record events
 *
 * Sets whether events are currently being recorded.
 */
void
sagarmatha_perf_log_set_enabled (SagarmathaPerfLog *perf_log,
                            gboolean      enabled)
{
...
static SagarmathaPerfEvent *
define_event (SagarmathaPerfLog *perf_log,
              const char   *name,
              ...
              const char   *signature)
{
  ...
  event = g_slice_new (SagarmathaPerfEvent);
...
/**
 * sagarmatha_perf_log_define_event:
 * @perf_log: a #SagarmathaPerfLog
 ...
 */
void
sagarmatha_perf_log_define_event (SagarmathaPerfLog *perf_log,
                             const char   *name,
                             const char   *description,
                             const char   *signature)
{
...
static SagarmathaPerfEvent *
lookup_event (SagarmathaPerfLog *perf_log,
              const char   *name,
              const char   *signature)
{
  SagarmathaPerfEvent *event = g_hash_table_lookup (perf_log->events_by_name, name);
...
static void
record_event (SagarmathaPerfLog   *perf_log,
              gint64          event_time,
              SagarmathaPerfEvent *event,
              const guchar   *bytes,
              size_t          bytes_len)
{
  SagarmathaPerfBlock *block;
  size_t total_bytes;
  guint32 time_delta;
  guint32 pos;
  ...
  if (perf_log->blocks->tail == NULL ||
      total_bytes + ((SagarmathaPerfBlock *)perf_log->blocks->tail->data)->bytes > BLOCK_SIZE)
    {
      block = g_new (SagarmathaPerfBlock, 1);
      block->bytes = 0;
      g_queue_push_tail (perf_log->blocks, block);
    }
  else
    {
      block = (SagarmathaPerfBlock *)perf_log->blocks->tail->data;
    }
 ...
 */
void
sagarmatha_perf_log_event (SagarmathaPerfLog *perf_log,
                      const char   *name)
{
  SagarmathaPerfEvent *event = lookup_event (perf_log, name, "");
  if (G_UNLIKELY (event == NULL))
    return;
 ...
 */
void
sagarmatha_perf_log_event_i (SagarmathaPerfLog *perf_log,
                        const char   *name,
                        gint32        arg)
{
  SagarmathaPerfEvent *event = lookup_event (perf_log, name, "i");
  if (G_UNLIKELY (event == NULL))
    return;
 ...
 */
void
sagarmatha_perf_log_event_x (SagarmathaPerfLog *perf_log,
                        const char   *name,
                        gint64        arg)
{
  SagarmathaPerfEvent *event = lookup_event (perf_log, name, "x");
  if (G_UNLIKELY (event == NULL))
    return;
 ...
 */
void
sagarmatha_perf_log_event_s (SagarmathaPerfLog *perf_log,
                         const char   *name,
                         const char   *arg)
{
  SagarmathaPerfEvent *event = lookup_event (perf_log, name, "s");
  if (G_UNLIKELY (event == NULL))
    return;
...
/**
 * sagarmatha_perf_log_define_statistic:
 * @name: name of the statistic and of the corresponding event.
 *  This should follow the same guidelines as for sagarmatha_perf_log_define_event()
 ...
 * sagarmatha_perf_log_collect_statistics() is called explicitly.
 *
 ...
 * such as sagarmatha_perf_log_update_statistic_i(). This can be done
 * at any time, but would normally done inside a function registered
 * with sagarmatha_perf_log_add_statistics_callback(). These functions
 * are called immediately before statistics are recorded.
 */
void
sagarmatha_perf_log_define_statistic (SagarmathaPerfLog *perf_log,
                                 const char   *name,
                                 const char   *description,
                                 const char   *signature)
{
  SagarmathaPerfEvent *event;
  SagarmathaPerfStatistic *statistic;
  ...
  statistic = g_slice_new (SagarmathaPerfStatistic);
...
static SagarmathaPerfStatistic *
lookup_statistic (SagarmathaPerfLog *perf_log,
                  const char   *name,
                  const char   *signature)
{
  SagarmathaPerfStatistic *statistic = g_hash_table_lookup (perf_log->statistics_by_name, name);
...
/**
 * sagarmatha_perf_log_update_statistic_i:
 * @perf_log: a #SagarmathaPerfLog
 ...
 */
void
sagarmatha_perf_log_update_statistic_i (SagarmathaPerfLog *perf_log,
                                   const char   *name,
                                   gint32        value)
{
  SagarmathaPerfStatistic *statistic;
...
/**
 * sagarmatha_perf_log_update_statistic_x:
 * @perf_log: a #SagarmathaPerfLog
 * @name: name of the statistic
 * @value: new value for the statistic
 *
 ...
 */
void
sagarmatha_perf_log_update_statistic_x (SagarmathaPerfLog *perf_log,
                                   const char   *name,
                                   gint64        value)
{
  SagarmathaPerfStatistic *statistic;
...
/**
 * sagarmatha_perf_log_add_statistics_callback:
 * @perf_log: a #SagarmathaPerfLog
 ...
 * and call a function such as sagarmatha_perf_log_update_statistic_i()
 * to update the value that will be recorded.
 */
void
sagarmatha_perf_log_add_statistics_callback (SagarmathaPerfLog               *perf_log,
                                        SagarmathaPerfStatisticsCallback callback,
                                        gpointer                    user_data,
                                        GDestroyNotify              notify)
{
  SagarmathaPerfStatisticsClosure *closure = g_slice_new (SagarmathaPerfStatisticsClosure);
...
/**
 * sagarmatha_perf_log_collect_statistics:
 * @perf_log: a #SagarmathaPerfLog
 *
 * Calls all the update functions added with
 * sagarmatha_perf_log_add_statistics_callback() and then records events
 * for all statistics, followed by a perf.statisticsCollected event.
 */
void
sagarmatha_perf_log_collect_statistics (SagarmathaPerfLog *perf_log)
{
  ...
  for (i = 0; i < perf_log->statistics_closures->len; i++)
    {
      SagarmathaPerfStatisticsClosure *closure;
  ...
  for (i = 0; i < perf_log->statistics->len; i++)
    {
      SagarmathaPerfStatistic *statistic = g_ptr_array_index (perf_log->statistics, i);
 ...
 */
void
sagarmatha_perf_log_replay (SagarmathaPerfLog            *perf_log,
                       ...
                       gpointer                 user_data)
{
  ...
  for (iter = perf_log->blocks->head; iter; iter = iter->next)
    {
      SagarmathaPerfBlock *block = iter->data;
      guint32 pos = 0;
...
/**
 * sagarmatha_perf_log_dump_events:
 * @perf_log: a #SagarmathaPerfLog
 ...
 */
gboolean
sagarmatha_perf_log_dump_events (SagarmathaPerfLog   *perf_log,
                            GOutputStream  *out,
                            GError        **error)
{
  GString *output;
  ...
  for (i = 0; i < perf_log->events->len; i++)
    {
      SagarmathaPerfEvent *event = g_ptr_array_index (perf_log->events, i);
 ...
 */
gboolean
sagarmatha_perf_log_dump_log (SagarmathaPerfLog   *perf_log,
                         GOutputStream  *out,
                         GError        **error)
{
  ...
  sagarmatha_perf_log_replay (perf_log, replay_to_json, &closure);
