...
static void
sagarmatha_dbus_init (gboolean replace)
{
  GError *error = NULL;
  ...
  if (!dbus_g_proxy_call (bus, "RequestName", &error,
                          ...
                          G_TYPE_INVALID))
    {
      g_printerr ("failed to acquire org.Sagarmatha: %s\n", error->message);
      exit (1);
    }
...
static void
malloc_statistics_callback (SagarmathaPerfLog *perf_log,
                            gpointer      data)
{
#ifdef HAVE_MALLINFO
  struct mallinfo info = mallinfo ();

  sagarmatha_perf_log_update_statistic_i (perf_log,
                                     "malloc.arenaSize",
                                     info.arena);
  sagarmatha_perf_log_update_statistic_i (perf_log,
                                     "malloc.mmapSize",
...
static void
sagarmatha_perf_log_init (void)
{
  SagarmathaPerfLog *perf_log = sagarmatha_perf_log_get_default ();
   ...
   */
  sagarmatha_perf_log_define_statistic (perf_log,
                                   "malloc.arenaSize",
  ...
  sagarmatha_perf_log_add_statistics_callback (perf_log,
...
print_version (const gchar    *option_name,
               ...
               GError        **error)
{
  g_print ("Sagarmatha %s\n", VERSION);
  exit (0);
}

GOptionEntry gnome_sagarmatha_options[] = {
  {
...
main (int argc, char **argv)
{
  ...
  g_option_context_add_main_entries (ctx, gnome_sagarmatha_options, GETTEXT_PACKAGE);
  ...
  meta_plugin_type_register (gnome_sagarmatha_plugin_get_type ());
  ...
  sagarmatha_dbus_init (meta_get_replace_current_wm ());
