...
struct _SagarmathaRecorderSrcClass
{
...
GST_BOILERPLATE(SagarmathaRecorderSrc, sagarmatha_recorder_src, GstPushSrc, GST_TYPE_PUSH_SRC);

static void
sagarmatha_recorder_src_init (SagarmathaRecorderSrc      *src,
			 SagarmathaRecorderSrcClass *klass)
{
...
static void
sagarmatha_recorder_src_base_init (gpointer klass)
{
}

static gboolean
sagarmatha_recorder_src_memory_used_update_idle (gpointer data)
{
  SagarmathaRecorderSrc *src = data;
 ...
 */
static void
sagarmatha_recorder_src_update_memory_used (SagarmathaRecorderSrc *src,
				       int               delta)
{
  g_mutex_lock (src->mutex);
  src->memory_used += delta;
  if (src->memory_used_update_idle == 0)
    src->memory_used_update_idle = g_idle_add (sagarmatha_recorder_src_memory_used_update_idle, src);
  g_mutex_unlock (src->mutex);
}
			   ...
			   GstBuffer  **buffer_out)
{
  ...
  sagarmatha_recorder_src_update_memory_used (src,
...
static void
sagarmatha_recorder_src_set_caps (SagarmathaRecorderSrc *src,
			     const GstCaps    *caps)
{
  if (caps == src->caps)
    return;
...
sagarmatha_recorder_src_finalize (GObject *object)
{
  ...
  sagarmatha_recorder_src_set_caps (src, NULL);
...
static void
sagarmatha_recorder_src_set_property (GObject      *object,
				 guint         prop_id,
				 const GValue *value,
				 GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CAPS:
      sagarmatha_recorder_src_set_caps (src, gst_value_get_caps (value));
      break;
    default:
...
static void
sagarmatha_recorder_src_get_property (GObject         *object,
				 guint            prop_id,
				 GValue          *value,
				 GParamSpec      *pspec)
{
...
static void
sagarmatha_recorder_src_class_init (SagarmathaRecorderSrcClass *klass)
{
  ...
  object_class->finalize = sagarmatha_recorder_src_finalize;
  object_class->set_property = sagarmatha_recorder_src_set_property;
  object_class->get_property = sagarmatha_recorder_src_get_property;

  push_src_class->create = sagarmatha_recorder_src_create;
...
/**
 * sagarmatha_recorder_src_add_buffer:
 *
 ...
 */
void
sagarmatha_recorder_src_close (SagarmathaRecorderSrc *src)
{
 ...
 */
void
sagarmatha_recorder_src_register (void)
{
  ...
  gst_plugin_register_static (GST_VERSION_MAJOR, GST_VERSION_MINOR,
			      "sagarmatharecorder",
			      "Plugin for SagarmathaRecorder",
			      plugin_init,
			      "0.1",
			      "LGPL",
			      "sagarmatha", "sagarmatha", "http://sagarmatha.chitwanix.com");
