...
struct _SagarmathaAppSystemPrivate {
  GMenuTree *apps_tree;
...
static void sagarmatha_app_system_finalize (GObject *object);
static void on_apps_tree_changed_cb (GMenuTree *tree, gpointer user_data);

G_DEFINE_TYPE(SagarmathaAppSystem, sagarmatha_app_system, G_TYPE_OBJECT);

static void sagarmatha_app_system_class_init(SagarmathaAppSystemClass *klass)
{
  ...
  g_type_class_add_private (gobject_class, sizeof (SagarmathaAppSystemPrivate));
}
...
setup_merge_dir_symlink(void)
{
    ...
    gchar *sym_path = g_build_filename (user_config, "menus", "sagarmatha-applications-merged", NULL);
...
static void
sagarmatha_app_system_init (SagarmathaAppSystem *self)
{
...
/* According to desktop spec, since our menu file is called 'sagarmatha-applications', our
 * merged menu folders need to be called 'sagarmatha-applications-merged'.  We'll setup the folder
 ...
 */
   ...
   * case by case.
   */
  priv->apps_tree = gmenu_tree_new ("sagarmatha-applications.menu", GMENU_TREE_FLAGS_INCLUDE_NODISPLAY);
...
sagarmatha_app_system_finalize (GObject *object)
{
  ...
  G_OBJECT_CLASS (sagarmatha_app_system_parent_class)->finalize (object);
}
      ...
      if (app != NULL)
        {
           ...
           * the new data.
           */
          old_entry = sagarmatha_app_get_tree_entry (app);
          gmenu_tree_item_ref (old_entry);
          _sagarmatha_app_set_entry (app, entry);
          g_object_ref (app);  /* Extra ref, removed in _replace below */
        }
      else
        {
          old_entry = NULL;
          app = _sagarmatha_app_new (entry);
        }
...
/**
 * sagarmatha_app_system_get_tree:
 *
 * Return Value: (transfer none): The #GMenuTree for apps
 */
GMenuTree *
sagarmatha_app_system_get_tree (SagarmathaAppSystem *self)
{
  return self->priv->apps_tree;
}

/**
 * sagarmatha_app_system_get_settings_tree:
 *
 * Return Value: (transfer none): The #GMenuTree for apps
 ...
 */
GMenuTree *
sagarmatha_app_system_get_settings_tree (SagarmathaAppSystem *self)
{
  return self->priv->settings_tree;
}
/**
 * sagarmatha_app_system_lookup_setting:
 *
 ...
 */
SagarmathaApp *
sagarmatha_app_system_lookup_setting (SagarmathaAppSystem *self,
                                 const char     *id)
{
  SagarmathaApp *app;
  /* Actually defer to the main app set if there's overlap */
  app = sagarmatha_app_system_lookup_app (self, id);
  if (app != NULL)
    return app;
  return NULL;
}
...
/**
 * sagarmatha_app_system_get_default:
 *
 * Return Value: (transfer none): The global #SagarmathaAppSystem singleton
 */
SagarmathaAppSystem *
sagarmatha_app_system_get_default ()
{
...
/**
 * sagarmatha_app_system_lookup_app:
 *
 * Find a #SagarmathaApp corresponding to an id.
 *
 * Return value: (transfer none): The #SagarmathaApp for id, or %NULL if none
 */
SagarmathaApp *
sagarmatha_app_system_lookup_app (SagarmathaAppSystem   *self,
                             const char       *id)
{
...
/**
 * sagarmatha_app_system_lookup_settings_app:
 *
 * Return value: (transfer none): The #SagarmathaApp for id, or %NULL if none
 * OBSOLETE - ONLY LEFT IN FOR COMPATIBILITY
 * RETURNS NULL
 ...
 */
SagarmathaApp *
sagarmatha_app_system_lookup_settings_app (SagarmathaAppSystem   *self,
                             const char       *id)
{
  return NULL;
}
...
/**
 * sagarmatha_app_system_lookup_app_by_tree_entry:
 * @system: a #SagarmathaAppSystem
 * @entry: a #GMenuTreeEntry
 *
 * Find a #SagarmathaApp corresponding to a #GMenuTreeEntry.
 *
 * Return value: (transfer none): The #SagarmathaApp for @entry, or %NULL if none
 */
SagarmathaApp *
sagarmatha_app_system_lookup_app_by_tree_entry (SagarmathaAppSystem  *self,
                                           GMenuTreeEntry  *entry)
{
  /* If we looked up directly in ->entry_to_app, we'd lose the
   * override of running apps.  Thus, indirect through the id.
   */
  return sagarmatha_app_system_lookup_app (self, gmenu_tree_entry_get_desktop_file_id (entry));
}


/**
 * sagarmatha_app_system_lookup_settings_app_by_tree_entry:
 *
 * Return value: (transfer none): The #SagarmathaApp for @entry, or %NULL if none
 * OBSOLETE - ONLY LEFT IN FOR COMPATIBILITY
 * RETURNS NULL
 ...
 */
SagarmathaApp *
sagarmatha_app_system_lookup_settings_app_by_tree_entry (SagarmathaAppSystem  *self,
                                           GMenuTreeEntry  *entry)
{
  return NULL;
}
...
/**
 * sagarmatha_app_system_lookup_app_for_path:
 * @system: a #SagarmathaAppSystem
 * @desktop_path: (type utf8): UTF-8 encoded absolute file name
 *
 * Find or create a #SagarmathaApp corresponding to a given absolute file
 ...
 */
SagarmathaApp *
sagarmatha_app_system_lookup_app_for_path (SagarmathaAppSystem   *system,
                                      const char       *desktop_path)
{
  ...
  app = sagarmatha_app_system_lookup_app (system, basename);
  if (!app)
    return NULL;

  app_path = gmenu_tree_entry_get_desktop_file_path (sagarmatha_app_get_tree_entry (app));
...
/**
 * sagarmatha_app_system_lookup_heuristic_basename:
 * @system: a #SagarmathaAppSystem
 ...
 *
 * Returns: (transfer none): A #SagarmathaApp for @name
 */
SagarmathaApp *
sagarmatha_app_system_lookup_heuristic_basename (SagarmathaAppSystem *system,
                                            const char     *name)
{
  SagarmathaApp *result;
  GSList *prefix;

  result = sagarmatha_app_system_lookup_app (system, name);
  if (result != NULL)
    return result;

  result = sagarmatha_app_system_lookup_settings_app (system, name);
  if (result != NULL)
    return result;
  ...
  for (prefix = system->priv->known_vendor_prefixes; prefix; prefix = g_slist_next (prefix))
    {
      ...
      result = sagarmatha_app_system_lookup_app (system, tmpid);
      g_free (tmpid);
      if (result != NULL)
        return result;
    }
...
/**
 * sagarmatha_app_system_get_all:
 * @system:
 *
 * Returns: (transfer container) (element-type SagarmathaApp): All installed applications
 */
GSList *
sagarmatha_app_system_get_all (SagarmathaAppSystem  *self)
{
  GSList *result = NULL;
  GHashTableIter iter;
  gpointer key, value;
  ...
  while (g_hash_table_iter_next (&iter, &key, &value))
    {
      SagarmathaApp *app = value;
      
      if (!g_desktop_app_info_get_nodisplay (sagarmatha_app_get_app_info (app)))
...
void
_sagarmatha_app_system_notify_app_state_changed (SagarmathaAppSystem *self,
                                            SagarmathaApp       *app)
{
  SagarmathaAppState state = sagarmatha_app_get_state (app);
...
/**
 * sagarmatha_app_system_get_running:
 * @self: A #SagarmathaAppSystem
 *
 * Returns the set of applications which currently have at least one
 * open window in the given context.  The returned list will be sorted
 * by sagarmatha_app_compare().
 *
 * Returns: (element-type SagarmathaApp) (transfer container): Active applications
 */
GSList *
sagarmatha_app_system_get_running (SagarmathaAppSystem *self)
{
  gpointer key, value;
  GSList *ret;
  GHashTableIter iter;
  ...
  while (g_hash_table_iter_next (&iter, &key, &value))
    {
      SagarmathaApp *app = key;

      ret = g_slist_prepend (ret, app);
    }

  ret = g_slist_sort (ret, (GCompareFunc)sagarmatha_app_compare);
  ...
  for (iter = terms; iter; iter = iter->next)
    {
      ...
      normalized_terms = g_slist_prepend (normalized_terms, sagarmatha_util_normalize_and_casefold (term));
    }
  return normalized_terms;
}

static GSList *
search_tree (SagarmathaAppSystem *self,
             GSList         *terms,
             GHashTable     *apps)
{
  ...
  while (g_hash_table_iter_next (&iter, &key, &value))
    {
      ...
      SagarmathaApp *app = value;
      (void)id;
      _sagarmatha_app_do_match (app, normalized_terms,
                           &prefix_results,
                           &substring_results);
    }
...
/**
 * sagarmatha_app_system_initial_search:
 * @system: A #SagarmathaAppSystem
 * @terms: (element-type utf8): List of terms, logical AND
 *
 * Search through applications for the given search terms.
 *
 * Returns: (transfer container) (element-type SagarmathaApp): List of applications
 */
GSList *
sagarmatha_app_system_initial_search (SagarmathaAppSystem  *self,
                                 GSList          *terms)
{
...
/**
 * sagarmatha_app_system_subsearch:
 * @system: A #SagarmathaAppSystem
 * @previous_results: (element-type SagarmathaApp): List of previous results
 * @terms: (element-type utf8): List of terms, logical AND
 *
 * Search through a previous result set; for more information, see
 * js/ui/search.js.  Note the value of @prefs must be
 * the same as passed to sagarmatha_app_system_initial_search().  Note that returned
 * strings are only valid until a return to the main loop.
 *
 * Returns: (transfer container) (element-type SagarmathaApp): List of application identifiers
 */
GSList *
sagarmatha_app_system_subsearch (SagarmathaAppSystem   *system,
...
/**
 * sagarmatha_app_system_search_settings:
 * @system: A #SagarmathaAppSystem
 * @terms: (element-type utf8): List of terms, logical AND
 *
 * Search through settings for the given search terms.
 *
 * Returns: (transfer container) (element-type SagarmathaApp): List of setting applications
 */
GSList *
sagarmatha_app_system_search_settings (SagarmathaAppSystem  *self,
                                  GSList          *terms)
{
  GSList *null_list;
  return null_list;
