...
/**
 * SECTION:sagarmatha-stack
 * @short_description: Pure "Z-axis" container class
 *
 * A #SagarmathaStack draws its children on top of each other,
...
#include "sagarmatha-stack.h"

G_DEFINE_TYPE (SagarmathaStack,
               sagarmatha_stack,
               ST_TYPE_CONTAINER);

static void
sagarmatha_stack_paint (ClutterActor *actor)
{
  CLUTTER_ACTOR_CLASS (sagarmatha_stack_parent_class)->paint (actor);
...
static void
sagarmatha_stack_pick (ClutterActor       *actor,
                  const ClutterColor *pick)
{
  /* Chain up so we get a bounding box painted (if we are reactive) */
  CLUTTER_ACTOR_CLASS (sagarmatha_stack_parent_class)->pick (actor, pick);
...
static void
sagarmatha_stack_get_preferred_width (ClutterActor *actor,
                                 gfloat for_height,
                                 gfloat *min_width_p,
...
static void
sagarmatha_stack_class_init (SagarmathaStackClass *klass)
{
  ClutterActorClass *actor_class = CLUTTER_ACTOR_CLASS (klass);
  StWidgetClass *widget_class = ST_WIDGET_CLASS (klass);

  actor_class->paint = sagarmatha_stack_paint;
  actor_class->pick = sagarmatha_stack_pick;
  actor_class->get_preferred_width = sagarmatha_stack_get_preferred_width;
  actor_class->get_preferred_height = sagarmatha_stack_get_preferred_height;
  actor_class->allocate = sagarmatha_stack_allocate;

  widget_class->navigate_focus = sagarmatha_stack_navigate_focus;
}

static void
sagarmatha_stack_init (SagarmathaStack *actor)
{
}
