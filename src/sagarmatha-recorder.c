...
#include "sagarmatha-recorder-src.h"
#include "sagarmatha-recorder.h"
...
struct _SagarmathaRecorderClass
{
  GObjectClass parent_class;
};

struct _SagarmathaRecorder {
  GObject parent;
...
{
  SagarmathaRecorder *recorder;
  GstElement *pipeline;
  GstElement *src;
  int outfile;
};

static void recorder_set_stage    (SagarmathaRecorder *recorder,
                                   ClutterStage  *stage);
static void recorder_set_framerate (SagarmathaRecorder *recorder,
                                    int framerate);
static void recorder_set_pipeline (SagarmathaRecorder *recorder,
                                   const char    *pipeline);
static void recorder_set_filename (SagarmathaRecorder *recorder,
                                   const char    *filename);
...
G_DEFINE_TYPE(SagarmathaRecorder, sagarmatha_recorder, G_TYPE_OBJECT);
...
/* The default filename pattern. Example sagarmatha-20090311b-2.webm
 */
#define DEFAULT_FILENAME "sagarmatha-%d%u-%c.webm"
...
static void
sagarmatha_recorder_init (SagarmathaRecorder *recorder)
{
  /* Calling gst_init() is a no-op if GStreamer was previously initialized */
  gst_init (NULL, NULL);

  sagarmatha_recorder_src_register ();
...
sagarmatha_recorder_finalize (GObject  *object)
{
  ...
  G_OBJECT_CLASS (sagarmatha_recorder_parent_class)->finalize (object);
}
 ...
 */
static void
recorder_update_memory_used (SagarmathaRecorder *recorder,
                             gboolean       repaint)
{
  guint memory_used = 0;
  GSList *l;
...
{
  SagarmathaRecorder *recorder = data;
...
static void
recorder_add_redraw_timeout (SagarmathaRecorder *recorder)
{
...
static void
recorder_remove_redraw_timeout (SagarmathaRecorder *recorder)
{
  if (recorder->redraw_timeout != 0)
    {
...
static void
recorder_fetch_cursor_image (SagarmathaRecorder *recorder)
{
 ...
 */
static void
recorder_draw_cursor (SagarmathaRecorder *recorder,
                      GstBuffer     *buffer)
{
 ...
 */
static void
recorder_draw_buffer_meter (SagarmathaRecorder *recorder)
{
  int fill_level;
 ...
 */
static void
recorder_record_frame (SagarmathaRecorder *recorder)
{
  GstBuffer *buffer;
  guint8 *data;
  guint size;
...
static void
recorder_update_size (SagarmathaRecorder *recorder)
{
...
static void
recorder_queue_redraw (SagarmathaRecorder *recorder)
{
 ...
 */
static void
recorder_get_initial_cursor_position (SagarmathaRecorder *recorder)
{
 ...
 */
static void
recorder_update_pointer (SagarmathaRecorder *recorder)
{
  Display *xdisplay = clutter_x11_get_default_display ();
  Window xwindow = clutter_x11_get_stage_window (recorder->stage);
  Window root, child;
...
static void
recorder_add_update_pointer_timeout (SagarmathaRecorder *recorder)
{
...
static void
recorder_remove_update_pointer_timeout (SagarmathaRecorder *recorder)
{
  if (recorder->update_pointer_timeout)
    {
...
static void
recorder_set_stage (SagarmathaRecorder *recorder,
                    ClutterStage  *stage)
{
  ...
  if (recorder->current_pipeline)
    sagarmatha_recorder_close (recorder);
...
static void
recorder_set_framerate (SagarmathaRecorder *recorder,
                        int framerate)
{
...
static void
recorder_set_pipeline (SagarmathaRecorder *recorder,
                       const char    *pipeline)
{
...
static void
recorder_set_filename (SagarmathaRecorder *recorder,
                       const char    *filename)
{
...
static void
sagarmatha_recorder_set_property (GObject      *object,
                             guint         prop_id,
                             const GValue *value,
                             GParamSpec   *pspec)
{
...
static void
sagarmatha_recorder_get_property (GObject         *object,
                             guint            prop_id,
                             GValue          *value,
                             GParamSpec      *pspec)
{
...
static void
sagarmatha_recorder_class_init (SagarmathaRecorderClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->finalize = sagarmatha_recorder_finalize;
  gobject_class->get_property = sagarmatha_recorder_get_property;
  gobject_class->set_property = sagarmatha_recorder_set_property;
 ...
 * SagarmathaRecorderSrc element where we inject frames then additional elements
...
recorder_pipeline_add_source (RecorderPipeline *pipeline)
{
  ...
  if (sink_pad == NULL)
    {
      g_warning("SagarmathaRecorder: pipeline has no unlinked sink pad");
      goto out;
    }

  pipeline->src = gst_element_factory_make ("sagarmatharecordersrc", NULL);
  ...
  if (!src_pad)
    {
      g_warning("SagarmathaRecorder: can't get src pad to link into pipeline");
      goto out;
    }

  if (gst_pad_link (src_pad, sink_pad) != GST_PAD_LINK_OK)
    {
      g_warning("SagarmathaRecorder: can't link to sink pad");
      goto out;
    }
 ...
 */
static int
recorder_open_outfile (SagarmathaRecorder *recorder)
{
...
recorder_pipeline_add_sink (RecorderPipeline *pipeline)
{
  ...
  if (!sink_pad)
    {
      g_warning("SagarmathaRecorder: can't get sink pad to link pipeline output");
      goto out;
    }
 ...
 */
static void
recorder_pipeline_on_memory_used_changed (SagarmathaRecorderSrc *src,
                                          GParamSpec       *spec,
                                          RecorderPipeline *pipeline)
{
  SagarmathaRecorder *recorder = pipeline->recorder;
  if (!recorder)
    return;
...
static gboolean
recorder_open_pipeline (SagarmathaRecorder *recorder)
{
  RecorderPipeline *pipeline;
  ...
  if (pipeline->pipeline == NULL)
    {
      g_warning ("SagarmathaRecorder: failed to parse pipeline: %s", error->message);
      g_error_free (error);
      goto error;
    }
...
static void
recorder_close_pipeline (SagarmathaRecorder *recorder)
{
...
/**
 * sagarmatha_recorder_new:
 * @stage: The #ClutterStage
 *
 * Create a new #SagarmathaRecorder to record movies of a #ClutterStage
 *
 * Return value: The newly created #SagarmathaRecorder object
 */
SagarmathaRecorder     *
...
/**
 * sagarmatha_recorder_set_framerate:
 * @recorder: the #SagarmathaRecorder
...
/**
 * sagarmatha_recorder_set_filename:
 * @recorder: the #SagarmathaRecorder
 ...
 * %%: A literal percent
 *
 * The default value is 'sagarmatha-%d%u-%c.ogg'.
 */
void
...
/**
 * sagarmatha_recorder_set_pipeline:
 * @recorder: the #SagarmathaRecorder
 ...
 * output file. (See sagarmatha_recorder_set_filename().) However
 ...
 * is paused and will be continued when sagarmatha_recorder_record()
 * is next called.
 */
void
...
/**
 * sagarmatha_recorder_close:
 * @recorder: the #SagarmathaRecorder
 *
 * Stops recording. It's possible to call sagarmatha_recorder_record()
...
sagarmatha_recorder_close (SagarmathaRecorder *recorder)
{
  ...
  if (recorder->state == RECORDER_STATE_RECORDING)
    sagarmatha_recorder_pause (recorder);
...
/**
 * sagarmatha_recorder_is_recording:
 *
