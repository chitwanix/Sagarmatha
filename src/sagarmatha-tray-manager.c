...
#include "sagarmatha-tray-manager.h"
#include "na-tray-manager.h"

#include "sagarmatha-tray-icon.h"
#include "sagarmatha-embedded-window.h"
#include "sagarmatha-global.h"

struct _SagarmathaTrayManagerPrivate {
...
typedef struct {
  SagarmathaTrayManager *manager;
  GtkWidget *socket;
  GtkWidget *window;
  ClutterActor *actor;
} SagarmathaTrayManagerChild;
...
G_DEFINE_TYPE (SagarmathaTrayManager, sagarmatha_tray_manager, G_TYPE_OBJECT);

static guint sagarmatha_tray_manager_signals [LAST_SIGNAL] = { 0 };
...
{
  SagarmathaTrayManagerChild *child = data;
  ...
  g_slice_free (SagarmathaTrayManagerChild, child);
}

static void
sagarmatha_tray_manager_set_property(GObject         *object,
                                guint            prop_id,
                                const GValue    *value,
                                GParamSpec      *pspec)
{
...
static void
sagarmatha_tray_manager_get_property(GObject         *object,
                                guint            prop_id,
                                GValue          *value,
                                GParamSpec      *pspec)
{
...
sagarmatha_tray_manager_finalize (GObject *object)
{
  ...
  G_OBJECT_CLASS (sagarmatha_tray_manager_parent_class)->finalize (object);
}

static void
sagarmatha_tray_manager_class_init (SagarmathaTrayManagerClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  g_type_class_add_private (klass, sizeof (SagarmathaTrayManagerPrivate));

  gobject_class->finalize = sagarmatha_tray_manager_finalize;
  gobject_class->set_property = sagarmatha_tray_manager_set_property;
  gobject_class->get_property = sagarmatha_tray_manager_get_property;

  sagarmatha_tray_manager_signals[TRAY_ICON_ADDED] =
    g_signal_new ("tray-icon-added",
                  G_TYPE_FROM_CLASS (klass),
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (SagarmathaTrayManagerClass, tray_icon_added),
                  NULL, NULL,
                  g_cclosure_marshal_VOID__OBJECT,
                  G_TYPE_NONE, 1,
                  CLUTTER_TYPE_ACTOR);
  sagarmatha_tray_manager_signals[TRAY_ICON_REMOVED] =
    g_signal_new ("tray-icon-removed",
                  G_TYPE_FROM_CLASS (klass),
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (SagarmathaTrayManagerClass, tray_icon_removed),
                  NULL, NULL,
                  g_cclosure_marshal_VOID__OBJECT,
                  G_TYPE_NONE, 1,
                  CLUTTER_TYPE_ACTOR);
...
static void
sagarmatha_tray_manager_style_changed (StWidget *theme_widget,
                                  gpointer  user_data)
{
  SagarmathaTrayManager *manager = user_data;
...
void
sagarmatha_tray_manager_manage_stage (SagarmathaTrayManager *manager,
                                 ClutterStage     *stage,
                                 StWidget         *theme_widget)
{
  ...
  g_signal_connect (theme_widget, "style-changed",
                    G_CALLBACK (sagarmatha_tray_manager_style_changed), manager);
  sagarmatha_tray_manager_style_changed (theme_widget, manager);
}

static void
sagarmatha_tray_manager_child_on_realize (GtkWidget             *widget,
                                     SagarmathaTrayManagerChild *child)
{
                    ...
                    gpointer user_data)
{
  ...
  win = sagarmatha_embedded_window_new (manager->priv->stage);
  ...
  child = g_slice_new0 (SagarmathaTrayManagerChild);
  child->manager = manager;
  child->window = win;
  child->socket = socket;

  g_signal_connect (win, "realize",
                    G_CALLBACK (sagarmatha_tray_manager_child_on_realize), child);
...
static void
sagarmatha_tray_manager_child_redisplay (gpointer socket_pointer, gpointer child_pointer, gpointer user_data)
{
  SagarmathaTrayManagerChild *child = child_pointer;
...
{
  g_hash_table_foreach(manager->priv->icons, sagarmatha_tray_manager_child_redisplay, manager);
}
  ...
  if (child->actor != NULL)
    {
      ...
      */
      g_signal_emit (manager,
                     sagarmatha_tray_manager_signals[TRAY_ICON_REMOVED], 0,
                     child->actor);
    }
