...
PANEL_LAUNCHER_PATH = os.path.join(home, ".sagarmatha", "panel-launchers")
...
class LauncherEditor(ItemEditor):
    ui_file = '/usr/lib/sagarmatha-desktop-editor/launcher-editor.ui'
...
class DirectoryEditor(ItemEditor):
    ui_file = '/usr/lib/sagarmatha-desktop-editor/directory-editor.ui'
...
class PanelLauncherEditor(ItemEditor):
    ...
    def check_custom_path(self):
        ...
        if self.item_path is None or "sagarmatha-custom-launcher" not in self.item_path:
            i = 1
            while True:
                name = os.path.join(PANEL_LAUNCHER_PATH, 'sagarmatha-custom-launcher-' + str(i) + '.desktop')
...
class Main:
    def __init__(self):
        ...
        self.tree = GMenu.Tree.new("sagarmatha-applications.menu", GMenu.TreeFlags.INCLUDE_NODISPLAY)
