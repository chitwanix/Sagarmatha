...
class Module:
    def __init__(self, content_box):
        ...
        sidePage.add_widget(GSettingsComboBox(_("Action on title bar double-click"),
                                            "org.sagarmatha.desktop.wm.preferences", "action-double-click-titlebar", None,
        ...
        sidePage.add_widget(GSettingsComboBox(_("Action on title bar middle-click"),
                                            "org.sagarmatha.desktop.wm.preferences", "action-middle-click-titlebar", None,
                                            [(i, i.replace("-", " ").title()) for i in ('toggle-shade', 'toggle-maximize', 'toggle-maximize-horizontally', 'toggle-maximize-vertically', 'minimize', 'shade', 'menu', 'lower', 'none')]))
        sidePage.add_widget(GSettingsComboBox(_("Action on title bar right-click"),
                                            "org.sagarmatha.desktop.wm.preferences", "action-right-click-titlebar", None,
                                            [(i, i.replace("-", " ").title()) for i in ('toggle-shade', 'toggle-maximize', 'toggle-maximize-horizontally', 'toggle-maximize-vertically', 'minimize', 'shade', 'menu', 'lower', 'none')]))
        sidePage.add_widget(GSettingsComboBox(_("Window focus mode"),
                                            "org.sagarmatha.desktop.wm.preferences", "focus-mode", None,
        ...
        sidePage.add_widget(GSettingsComboBox(_("Modifier to use for modified window click actions"),
                                            "org.sagarmatha.desktop.wm.preferences", "mouse-button-modifier", None,
                                            [(i, i.title()) for i in ("","<Alt>","<Super>","<Control>")]))

        sidePage.add_widget(TitleBarButtonsOrderSelector())

        sidePage.add_widget(GSettingsSpinButton(_("Window drag/resize threshold"), "org.sagarmatha.muffin", "resize-threshold", None, 1, 100, 1, 1, _("Pixels")), True)

        sidePage.add_widget(GSettingsCheckButton(_("Attach dialog windows to their parent window's titlebar"), "org.sagarmatha.muffin", "attach-modal-dialogs", None))
        ...
        alttab_styles_combo = GSettingsComboBox(_("ALT-tab switcher style"), "org.sagarmatha", "alttab-switcher-style", None, alttab_styles)
        sidePage.add_widget(alttab_styles_combo)
        sidePage.add_widget(GSettingsCheckButton(_("Enforce displaying the alt-tab switcher on the primary monitor instead of the active one"), "org.sagarmatha", "alttab-switcher-enforce-primary-monitor", None))
        sidePage.add_widget(GSettingsCheckButton(_("Enable mouse-wheel scrolling in Window List applet"), "org.sagarmatha", "window-list-applet-scroll", None))
        sidePage.add_widget(GSettingsCheckButton(_("Bring windows which require attention to the current workspace (instead of switching to the window's workspace)"), "org.sagarmatha", "bring-windows-to-current-workspace", None))
        sidePage.add_widget(GSettingsCheckButton(_("Show an alert in the window list when a window from another workspace requires attention"), "org.sagarmatha", "window-list-applet-alert", None))


class TitleBarButtonsOrderSelector(Gtk.Table):
    def __init__(self):
        self.schema = "org.sagarmatha.muffin"
