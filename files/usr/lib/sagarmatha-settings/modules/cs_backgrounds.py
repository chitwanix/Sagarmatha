...
import sys
sys.path.append('/usr/lib/sagarmatha-settings/bin')
...
class BackgroundWallpaperPane (Gtk.VBox):
    ...
    def update_icon_view(self):
        pictures_list = []
        if os.path.exists("/usr/share/sagarmatha-background-properties"):
            for i in os.listdir("/usr/share/sagarmatha-background-properties"):
                if i.endswith(".xml"):
                    pictures_list += self.parse_xml_backgrounds_list(os.path.join("/usr/share/sagarmatha-background-properties", i))
        
        path = os.path.join(os.getenv("HOME"), ".sagarmatha", "backgrounds")
...
class BackgroundSlideshowPane(Gtk.Table):
    def __init__(self, sidepage, gnome_background_schema, sagarmatha_background_schema):
        Gtk.Table.__init__(self)
        self.set_col_spacings(5)
        self.set_row_spacings(5)
        
        self._sagarmatha_background_schema = sagarmatha_background_schema
        ...
        self.folder_selector.set_filename(self._sagarmatha_background_schema["slideshow-folder"])
        ...
        self.recursive_cb.set_active(self._sagarmatha_background_schema.get_boolean("slideshow-recursive"))
        ...
        self.delay_button.set_value(self._sagarmatha_background_schema.get_int("slideshow-delay"))
        self.delay_button.connect("value-changed", self._on_delay_changed)
    
    def _on_recursive_toggled(self, button):
        self._sagarmatha_background_schema.set_boolean("slideshow-recursive", button.get_active())
        self.update_list()
    
    def _on_delay_changed(self, button):
        self._sagarmatha_background_schema.set_int("slideshow-delay", int(button.get_value()))
        self.update_list()
    
    def _on_folder_selected(self, button):
        self._sagarmatha_background_schema.set_string("slideshow-folder", button.get_filename())
        self.update_list()
    ...
    def _do_update_list(self, folder, recursive, delay, transition_duration = 0):
        if os.path.exists(folder) and os.path.isdir(folder):
            ...
            if not os.path.exists(os.path.join(os.getenv("HOME"), ".sagarmatha", "backgrounds")):
                rec_mkdir(os.path.join(os.getenv("HOME"), ".sagarmatha", "backgrounds"))
            filename = os.path.join(os.getenv("HOME"), ".sagarmatha", "backgrounds", "slideshow.xml")
            f = open(filename, "w")
            f.write(xml_data)
            f.close()
            Gio.Settings("org.sagarmatha.desktop.background").set_string("picture-uri", "file://" + filename)

class BackgroundSidePage (SidePage):
    def __init__(self, name, icon, keywords, advanced, content_box):
        SidePage.__init__(self, name, icon, keywords, advanced, content_box, -1)
        self._gnome_background_schema = Gio.Settings("org.sagarmatha.desktop.background")
        self._sagarmatha_background_schema = Gio.Settings("org.sagarmatha.background")
        self._add_wallpapers_dialog = AddWallpapersDialog()
        
        self._sagarmatha_background_schema.connect("changed::mode", self._on_mode_changed)
    
    def _on_mode_changed(self, settings, key):
        for i in self.mainbox.get_children():
            self.mainbox.remove(i)
        if self._sagarmatha_background_schema["mode"] == "slideshow":
    ...
    def build(self, advanced):
        ...
        #self.background_mode = GSettingsComboBox("", "org.sagarmatha.background", "mode", None, BACKGROUND_MODES).content_widget
        ...
        self.slideshow_pane = BackgroundSlideshowPane(self, self._gnome_background_schema, self._sagarmatha_background_schema)
        if self._sagarmatha_background_schema["mode"] == "slideshow":
            self.mainbox.add(self.slideshow_pane)
        else:
            self.mainbox.add(self.wallpaper_pane)
        ...
        self.picture_options = GSettingsComboBox("", "org.sagarmatha.desktop.background", "picture-options", None, BACKGROUND_PICTURE_OPTIONS)
        advanced_options_box.pack_start(self.picture_options, False, False, 0)
        
        l = Gtk.Label(_("Gradient"))
        l.set_alignment(0, 0.5)
        advanced_options_box.pack_start(l, False, False, 0)
        self.color_shading_type = GSettingsComboBox("", "org.sagarmatha.desktop.background", "color-shading-type", None, BACKGROUND_COLOR_SHADING_TYPES)
        advanced_options_box.pack_start(self.color_shading_type, False, False, 0)
        
        hbox = Gtk.HBox()
        l = Gtk.Label(_("Colors"))
        hbox.pack_start(l, False, False, 2)
        self.primary_color = GSettingsColorChooser("org.sagarmatha.desktop.background", "primary-color", None)
        hbox.pack_start(self.primary_color, False, False, 2)
        self.secondary_color = GSettingsColorChooser("org.sagarmatha.desktop.background", "secondary-color", None)
    ...
    def _add_wallpapers(self):
        ...
        if filenames:
            dest_dir = os.path.join(os.getenv("HOME"), ".sagarmatha", "backgrounds")
