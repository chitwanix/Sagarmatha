try:
    ...
    # except:
    #     HAS_WEBKIT=False
    #     print "WebKit not found on this system. These packages are needed for adding summit:"
...
settings_dir = '%s/.sagarmatha/configs/' % home

URL_SUMMIT_HOME = "http://sagarmatha.chitwanix.com"
URL_SUMMIT_APPLET_LIST = URL_SUMMIT_HOME + "/json/applets.json"
URL_SUMMIT_THEME_LIST = URL_SUMMIT_HOME + "/json/themes.json"
URL_SUMMIT_DESKLET_LIST = URL_SUMMIT_HOME + "/json/desklets.json"
URL_SUMMIT_EXTENSION_LIST = URL_SUMMIT_HOME + "/json/extensions.json"
...
class Spice_Harvester:
    ...
    def show_detail(self, uuid, onSelect=None, onClose=None):        
        ...
        # Browsing the info within the app would be great (ala chitwanixinstall) but until it is fully ready 
        ...
        os.system("xdg-open '%s/%ss/view/%s'" % (URL_SUMMIT_HOME, self.collection_type, appletData['summit-id']))
        return
        ...
        if not os.path.exists(screenshot_path):
            ...
            self.download_url = URL_SUMMIT_HOME + appletData['screenshot']
            self.download_with_progressbar(f, screenshot_path, _("Downloading screenshot"), False)

        template = open(os.path.realpath(os.path.dirname(os.path.abspath(__file__)) + "/../data/summit/applet-detail.html")).read()
        subs = {}
    ...
    def get_index_url(self):
        if self.collection_type == 'applet':
            return URL_SUMMIT_APPLET_LIST
        elif self.collection_type == 'extension':
            return URL_SUMMIT_EXTENSION_LIST
        elif self.collection_type == 'theme':
            return URL_SUMMIT_THEME_LIST
        elif self.collection_type == 'desklet':
            return URL_SUMMIT_DESKLET_LIST
        else:
            return False

    def get_cache_folder(self):
        cache_folder = "%s/.sagarmatha/summit.cache/%s/" % (home, self.collection_type)
    ...
    def get_install_folder(self):
        if self.collection_type in ['applet','desklet','extension']:
            install_folder = '%s/.local/share/sagarmatha/%ss/' % (home, self.collection_type)
        elif self.collection_type == 'theme':
    ...
    def refresh_cache(self, load_assets=True):
        ...
        self.load_cache()
        #print "Loaded index, now we know about %d summit." % len(self.index_cache)
    ...
    def load_cache(self):
        ...
        try:
            ...
            try:
                ...
                pass
            self.errorMessage(_("Something went wrong with the summit download.  Please try refreshing the list again."), str(detail))
    ...
    def load_assets(self):
        ...
        for uuid in uuids:
            ...
            if not os.path.isfile(icon_path):
                ...
                f = open(icon_path, 'w')
                if not self.themes:
                    self.download_url = URL_SUMMIT_HOME + self.index_cache[uuid]['icon']
                else:
                    self.download_url = URL_SUMMIT_HOME + "/uploads/themes/thumbs/" + self.index_cache[uuid]['icon_filename']
                valid = True
                try:
    ...
    def install(self, uuid, is_update, is_active):
        ...
        self.download_url = URL_SUMMIT_HOME + self.index_cache[uuid]['file'];
        ...
        if not self.themes:
            ...
            try:
                ...
                for file in self.get_members(zip):
                    if not file.filename.endswith('/') and ((file.external_attr >> 16L) & 0o755) == 0o755:
                        ...
                        if os.path.exists("/usr/bin/gksu") and os.path.exists("/usr/lib/sagarmatha-settings/bin/installSchema.py"):
                            launcher = "gksu  --message \"<b>%s</b>\"" % sentence
                            tool = "/usr/lib/sagarmatha-settings/bin/installSchema.py %s" % (os.path.join(dirname, file.filename))
            ...
            try:
                ...
                # Check dir name - it may or may not be the same as the theme name from our summit data
                ...
                # Test for correct folder structure - look for sagarmatha.css
                file = open(os.path.join(temp_path, "sagarmatha", "sagarmatha.css"), 'r')
                file.close()
                ...
                md["uuid"] = uuid
                raw_meta = json.dumps(md, indent=4)
                file = open(os.path.join(temp_path, "sagarmatha", "metadata.json"), 'w+')
                file.write(raw_meta)
                file.close()
        ...
        try:
            if not self.themes:
                if schema_filename != "":
                    ...
                    if os.path.exists("/usr/bin/gksu") and os.path.exists("/usr/lib/sagarmatha-settings/bin/removeSchema.py"):
                        launcher = "gksu  --message \"<b>%s</b>\"" % sentence
                        tool = "/usr/lib/sagarmatha-settings/bin/removeSchema.py %s" % (schema_filename)
                        command = "%s %s" % (launcher, tool)
                        os.system(command)
                    else:
