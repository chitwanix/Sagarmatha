...
class ExtensionSidePage (SidePage):
    ...
    def build(self, advanced):
        ...
        if not self.themes:
            self.settings = Gio.Settings.new("org.sagarmatha")
            self.enabled_extensions = self.settings.get_strv("enabled-%ss" % (self.collection_type))
        else:
            self.settings = Gio.Settings.new("org.sagarmatha.theme")
        ...
        pb = GdkPixbuf.Pixbuf.new_from_file_at_size("/usr/lib/sagarmatha-settings/data/update.svg", w, h)
    ...
    def _is_active_data_func(self, column, cell, model, iter, data=None):
        ...
        if (enabled):
            if not self.themes:
                img = GdkPixbuf.Pixbuf.new_from_file_at_size("/usr/lib/sagarmatha-settings/data/running.svg", ROW_SIZE, ROW_SIZE)
            else:
                img = GdkPixbuf.Pixbuf.new_from_file_at_size("/usr/lib/sagarmatha-settings/data/installed.svg", ROW_SIZE, ROW_SIZE)
        else:
            img = GdkPixbuf.Pixbuf.new_from_file_at_size("/usr/lib/sagarmatha-settings/data/inactive.png", ROW_SIZE, ROW_SIZE)
    ...
    def _gm_status_data_func(self, column, cell, model, iter, data=None):
        ...
        if installed:
            if can_update:
                img = GdkPixbuf.Pixbuf.new_from_file_at_size("/usr/lib/sagarmatha-settings/data/update.svg", ROW_SIZE, ROW_SIZE)
    ...
    def _restore_default_extensions(self):
        if not self.themes:
            os.system(('gsettings reset org.sagarmatha next-%s-id') % (self.collection_type))
            os.system(('gsettings reset org.sagarmatha enabled-%ss') % (self.collection_type))
        else:
            os.system("gsettings reset org.sagarmatha.theme name")
    ...
    def load_extensions(self):
        self.model.clear()
        if not self.themes:
            self.load_extensions_in(('%s/.local/share/sagarmatha/%ss') % (home, self.collection_type))
            self.load_extensions_in(('/usr/share/sagarmatha/%ss') % (self.collection_type))
        else:
                    ...
                    try:
                        if os.path.exists("%s/%s/metadata.json" % (directory, extension)):
                            ...
                            if self.search_entry.get_text().upper() in (extension_name + extension_description).upper():
                                ...
                                if img is None:                                                
                                    img = GdkPixbuf.Pixbuf.new_from_file_at_size( ("/usr/lib/sagarmatha-settings/data/icons/%ss.svg") % (self.collection_type), ROW_SIZE, ROW_SIZE)
                                ...
                                if (os.access(directory, os.W_OK)):
                                    img = GdkPixbuf.Pixbuf.new_from_file_at_size("/usr/lib/sagarmatha-settings/data/user.png", ROW_SIZE, ROW_SIZE)
                                else:
                                    img = GdkPixbuf.Pixbuf.new_from_file_at_size("/usr/lib/sagarmatha-settings/data/system.svg", ROW_SIZE, ROW_SIZE)
                ...
                for theme in themes:
                    ...
                    try:
                        if stock_theme:
                            ...
                            path = os.path.join(directory, theme, "sagarmatha")
                        if os.path.exists(path) and os.path.isdir(path):
                            ...
                            if os.path.exists(os.path.join(path, "thumbnail.png")):
                                ...
                                icon_path = "/usr/lib/sagarmatha-settings/data/icons/themes.svg"
