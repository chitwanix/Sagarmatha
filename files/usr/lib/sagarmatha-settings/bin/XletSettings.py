...
class XletSetting:
    ...
    def __init__(self, uuid, parent, _type):
        ...
        self.builder.add_from_file("/usr/lib/sagarmatha-settings/bin/xlet-settings.ui")
    ...
    def load_applet_data (self, uuid):
        found = self.get_meta_data_for_applet("/usr/share/sagarmatha/%ss/%s" % (self.type, uuid))
        if not found:
            found = self.get_meta_data_for_applet("%s/.local/share/sagarmatha/%ss/%s" % (home, self.type, uuid))
        if not found:
            print("Could not find %s metadata - are you sure it's installed correctly?" % self.type)
            return
        found = self.get_settings_for_applet("%s/.sagarmatha/configs/%s" % (home, uuid))
        if not found:
    ...
    def on_highlight_button_clicked(self, widget):
        sagarmatha_dbus = self.session_bus.get_object("org.Sagarmatha", "/org/Sagarmatha")
        highlight_applet = sagarmatha_dbus.get_dbus_method('highlightApplet', 'org.Sagarmatha')
