...
# - if sagarmatha --replace was called from Melange, it will be killed when this process is closed
...
MELANGE_DBUS_NAME = "org.Sagarmatha.Melange"
MELANGE_DBUS_PATH = "/org/Sagarmatha/Melange"
...
class NewLogDialog(Gtk.Dialog):
    def __init__(self, parent):
        ...
        self.store.append(["glass.log", "~/.sagarmatha/glass.log"])
...
class SagarmathaLog(dbus.service.Object):
    def __init__ (self):
        self.window = None
    ...
    def run(self):
        ...
        label.set_markup("<u>Melange - Sagarmatha Debugger</u> ")
        label.show()
        ...
        statusLabel.set_markup(" <span foreground='red'>[ Sagarmatha is OFFLINE! ]</span> ")
        statusLabel.set_tooltip_text("The connection to sagarmatha is broken")
    ...
    def createActionButton(self):
        ...
        menu.append(Gtk.SeparatorMenuItem())
        menu.append(self.createMenuItem('Restart Sagarmatha', self.onRestartClicked))
        menu.append(self.createMenuItem('Reset Sagarmatha Settings', self.onResetClicked))
        menu.append(Gtk.SeparatorMenuItem())
    ...
    def onResetClicked(self, menuItem):
        dialog = Gtk.MessageDialog(self.window, 0,
                                   ...
                                   "Reset all sagarmatha settings to default?")
        dialog.set_title("Warning: Trying to reset all sagarmatha settings!")

        response = dialog.run()
        dialog.destroy()
        if response == Gtk.ResponseType.YES:
            os.system("gsettings reset-recursively org.sagarmatha &")
