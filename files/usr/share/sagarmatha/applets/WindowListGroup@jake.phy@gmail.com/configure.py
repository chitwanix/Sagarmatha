...
def Settings(schema):
    path = os.path.expanduser("~/.local/share/sagarmatha/applets/WindowListGroup@jake.phy@gmail.com/schemas/")
...
class SagarmathaListSettings:
	...
	self.NoteBK = Gtk.Notebook()
        ...
        self.compile_schema =  "glib-compile-schemas ~/.local/share/sagarmatha/applets/WindowListGroup@jake.phy@gmail.com/schemas"
        self.copy_applet = "mkdir -p ~/.local/share/sagarmatha/applets/WindowListGroup@jake.phy@gmail.com && cp -f -v  convenience.js metadata.json applet.js configure.py icon.png specialButtons.js specialMenus.js ~/.local/share/sagarmatha/applets/WindowListGroup@jake.phy@gmail.com"
        self.copy_schema = "mkdir -p ~/.local/share/sagarmatha/applets/WindowListGroup@jake.phy@gmail.com/schemas && cp -f -v  org.sagarmatha.applets.windowListGroup.gschema.xml ~/.local/share/sagarmatha/applets/WindowListGroup@jake.phy@gmail.com/schemas"
        #self.copy_schema = "gksu 'cp -f -v org.sagarmatha.applets.windowListGroup.gschema.xml /usr/share/glib-2.0/schemas/'"
        self.remove_applet = "rm -rf -v ~/.local/share/sagarmatha/applets/WindowListGroup@jake.phy@gmail.com"
        self.restart_sagarmatha = "nohup sagarmatha --replace > /dev/null 2>&1 &"
	...
	self.installation = NewLabel('Installation')
        ...
        self.restart_sagarmatha = GTKButton("Restart Sagarmatha", 'Restart Sagarmatha', self.restart_sagarmatha, "Restart Sagarmatha for applet to work")

        self.Ivbox = Gtk.VBox()
        self.Ivbox.add(self.installation)
        self.Ivbox.add(self.install_applet)
        self.Ivbox.add(self.uninstall_applet)
        self.Ivbox.add(self.restart_sagarmatha)
	self.NoteBK.append_page(self.Ivbox, Gtk.Label("Install"))

	if Settings("org.sagarmatha.applets.windowListGroup") != "no Schema":
	    self.window_list_settings = NewLabel('WINDOW LIST SETTINGS')
	    self.list_title_display = GSettingsComboBox('Window Title Display', "org.sagarmatha.applets.windowListGroup", 'title-display', [(0, 'none'), (1, 'app'), (2, 'title'), (3, 'focused')], "title: display the window title, app: diplay app name, none: don't display anything")
            self.list_group_apps = GSettingsCheckButton("Group Apps into one Icon", "org.sagarmatha.applets.windowListGroup", "group-apps", "Checked: group windows into one app icon, else: every window has it's own icon")
            self.seperate_favorites = GSettingsComboBox('Favorites Display', "org.sagarmatha.applets.windowListGroup", 'favorites-display', [(0, 'favorites'), (1, 'pinned'), (2, 'none')], "favorites: display the favorites, pinned: display pinned apps instead of favorites, none: don't display anything")
	    self.list_number_display = GSettingsComboBox('List Number Display', "org.sagarmatha.applets.windowListGroup", 'number-display', [(0, 'smart'), (1, 'normal'), (2, 'none')], "normal: display window number, smart: display window number if more than one window, none: don't display number")
	    self.thumbnail_settings = NewLabel('THUMBNAIL SETTINGS')
            self.thumbnail_size = GSettingsSpinButton("Size of Thumbnails", "org.sagarmatha.applets.windowListGroup", "thumbnail-size", 5, 30, 1, 1, "Thumbnail Size; Default is ten")
            self.thumbnail_timeout = GSettingsSpinButton("Thumbnail Timeout", "org.sagarmatha.applets.windowListGroup", "thumbnail-timeout", 0, 2000, 100, 1000, "Thumbnail timeout in milliseconds")
	    self.sort_thumnails = GSettingsComboBox('Sort Thumbnails', "org.sagarmatha.applets.windowListGroup", 'sort-thumbnails', [(0, 'Last focused'), (1, 'Order opened')], "opened: sort by first opened, focused: sort by last focused")
	    self.hover_peek_settings = NewLabel('HOVER PEEK SETTINGS')
            self.hover_peek = GSettingsCheckButton('Enable Hover Peek', "org.sagarmatha.applets.windowListGroup", "enable-hover-peek", "Checked: enable hover peek, else: disable it")
            self.window_opacity = GSettingsSpinButton("Window Opacity", "org.sagarmatha.applets.windowListGroup", "hover-peek-opacity", 0, 255, 10, 100, "Opacity of the windows when peeked")
            self.peek_time = GSettingsSpinButton("Fade in/out Time", "org.sagarmatha.applets.windowListGroup", "hover-peek-time", 0, 1000, 10, 1000, "Hover Peek Fade in/out time")
