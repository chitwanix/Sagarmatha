/**
 * Sagarmatha Screenshot class to support capture selection and 
...
CaptureDialog.prototype = {
   ...
   _init: function(helper) {
      ...
      this.radioGroup.addButton(SelectionType.SAGARMATHA, "Grab a UI element");
...
ScreenshotHelper.prototype = {
   _init: function(selectionType, callback, params) {
      ...
      this._xfixesCursor = Sagarmatha.XFixesCursor.get_for_stage(global.stage);
   ...
   playSound: function(effect) {
      // @todo: Maybe check against GLib.getenv("SAGARMATHA_VERSION") instead.
   ...
   runCaptureMode: function(mode) {
      ...
      else if (mode == SelectionType.SAGARMATHA) {
         this.selectSagarmatha();
      }
   ...
   selectSagarmatha: function() {
      ...
      this.container = new Sagarmatha.GenericContainer({
      ...
      this.captureTimer(this._params, Lang.bind(this, Lang.bind(this, function() {
         global.set_cursor(Sagarmatha.Cursor.POINTING_HAND);
         this.showInstructions();
         this._capturedEventId = global.stage.connect('captured-event', Lang.bind(this, this._onCapturedEvent));
      })));
   },

   _updateSagarmatha: function(event) {
   ...
   showInstructions: function() {
      ...
      else if (this._selectionType == SelectionType.SAGARMATHA) {
         startY -= 140;
   ...
   screenshotScreen: function(options) {
      ...
      let screenshot = new Sagarmatha.Screenshot();
   ...
   screenshotSagarmatha: function(actor, stageX, stageY, options) {
      //global.log('screenshotSagarmatha() [actor,stageX,stageY]');
   ...
   _onCapturedEvent: function(actor, event) {
      ...
      if (type == Clutter.EventType.KEY_PRESS) {
         ...
         if (this._selectionType == SelectionType.AREA) {
            ...
            if (this.getModifier(Clutter.Shift_L))
            {
               ...
               let [stageX, stageY] = event.get_coords();
               this.screenshotSagarmatha(this._target, stageX, stageY);
               //this.reset();
               return true;
            }
            return true;
         ...
         let titles = windows.map(function(w) {
             if (w['get_meta_window'])
                 ...
                 return 'Unknown Sagarmatha container';
         });
      ...
      else if (type == Clutter.EventType.MOTION && this._selectionType == SelectionType.SAGARMATHA) {
         this._updateSagarmatha(event);
      }
      else if (type == Clutter.EventType.SCROLL && this._selectionType == SelectionType.SAGARMATHA) {
         this.hideInstructions();
