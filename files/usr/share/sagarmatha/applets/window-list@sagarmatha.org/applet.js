...
const DEFAULT_ICON_SIZE = 16; // too bad this can't be defined in theme (sagarmatha-app.create_icon_texture returns a clutter actor, not a themable object -
...
//    __proto__ : AppMenuButton.prototype,
    ...
    _init: function(applet, metaWindow, animation, orientation, panel_height, draggable) {
        ...
        this._iconBox = new Sagarmatha.Slicer({ name: 'appMenuIcon' });
    ...
    _onButtonRelease: function(actor, event) {
        this._tooltip.hide();
        if (!this._draggable) {
            if ( Sagarmatha.get_event_state(event) & Clutter.ModifierType.BUTTON1_MASK ) {
        ...
        if ( Sagarmatha.get_event_state(event) & Clutter.ModifierType.BUTTON1_MASK ) {
            ...
            this._windowHandle(false);
        } else if (Sagarmatha.get_event_state(event) & Clutter.ModifierType.BUTTON2_MASK) {
            this.metaWindow.delete(global.get_current_time());
        } else if (Sagarmatha.get_event_state(event) & Clutter.ModifierType.BUTTON3_MASK) {
...
MyApplet.prototype = {
    ...
    _onWindowStateChange: function(sagarmathawm, actor) {
        for ( let i=0; i<this._windows.length; ++i ) {
