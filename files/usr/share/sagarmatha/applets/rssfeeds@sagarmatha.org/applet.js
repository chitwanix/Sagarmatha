const UUID = "rssfeeds@sagarmatha.org"
...
/* Check if current Sagarmatha version is greater than or equal to a specific
 * version */
function sagarmatha_version_gte(version) {
...
FeedDisplayMenuItem.prototype = {
    ...
    _init: function (url, owner, params) {
        ...
        this.reader = new FeedReader.FeedReader(
                url,
                '~/.sagarmatha/' + UUID + '/' + owner.instance_id,
                {
...
FeedApplet.prototype = {
    ...
    build_context_menu: function() {
        ...
        /* Include setting menu item in Sagarmatha < 2.0.0 */
        if (!sagarmatha_version_gte('2.0.0')) {
            s = new Applet.MenuItem(
                    _("Settings"),
                    "emblem-system-symbolic",
                    Lang.bind(this, function() {
                        Util.spawnCommandLine('sagarmatha-settings applets ' + UUID);
                    }));
            s.icon.icon_type = St.IconType.SYMBOLIC;
            this._applet_context_menu.addMenuItem(s);
        }
